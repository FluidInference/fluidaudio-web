...
import directSemanticValidationTensors from "./generated/direct-semantic-validation-tensors.json";
...
const PLANNER_TOKENIZER_ALIASES = Object.freeze([
  ...
  ["assets/planner/tokenizer_config.json", "assets/qwen/tokenizer_config.json"],
  ["assets/planner/chat_template.jinja", "assets/qwen/chat_template.jinja"],
] as const);

const FORBIDDEN_DIRECT_FILE = /^(?:assets\/planner\/|weights\/(?:planner|semantic)\/|licenses\/ACE-Step-acestep-5Hz-lm-0\.6B-README\.md$)/u;
...
type ManifestLoader = (configuration: AceWorkerConfiguration, signal: AbortSignal) => Promise<AceLoadedPackageManifest>;

type ModelAcquirer = (options: DirectOnlyAcquireOptions) => Promise<AceAcquiredModelFiles>;

type TokenizerLoader = (kind: AceTokenizerKind, assets: AceTokenizerAssetBundle) => Promise<LoadedAceTokenizer>;
...
type StockBackendFactory = (options: AceWebGpuPipelineOptions) => AcePipelineBackend;
...
class DirectOnlyBackend implements AcePipelineBackend {
  ...
  async initialize(configuration: AceWorkerConfiguration, context: AceInitializationContext): Promise<AceRuntimeDiagnostics> {
    return await this.stock.initialize(configuration, context);
  }

  async generate(request: AceGenerationRequest, context: AceGenerationContext): Promise<AceGenerationResult> {
    if (request.planner.mode !== "disabled") {
      throw new DOMException("This ACE-Step demo supports direct generation only", "NotSupportedError");
    }
...
function installDirectOnlyDependencies(stock: AcePipelineBackend): void {
  ...
  try {
    ...
    throw new Error("ACE direct-only dependency seam is not writable", { cause: error });
  }
...
function requireDependencySeam(stock: AcePipelineBackend): DirectOnlyDependencySeam {
  ...
  if (
    ...
    throw new Error("ACE backend no longer exposes the pinned direct-only dependency seam");
  }
  return value as DirectOnlyDependencySeam;
}

function assertDirectOnlyManifest(loaded: AceLoadedPackageManifest): void {
  const forbiddenFile = loaded.manifest.files.find((file) => FORBIDDEN_DIRECT_FILE.test(file.name));
  const forbiddenTensor = Object.entries(loaded.manifest.tensors).find(([, tensor]) => tensor.phase === "planner" || tensor.phase === "semantic");
  const forbiddenSource = loaded.manifest.source.find((source) => source.repository === "ACE-Step/acestep-5Hz-lm-0.6B");
  if (forbiddenFile !== undefined || forbiddenTensor !== undefined || forbiddenSource !== undefined) {
    const identity = forbiddenFile?.name ?? forbiddenTensor?.[0] ?? forbiddenSource?.key ?? "unknown planner material";
    throw new Error(`ACE direct-only manifest contains forbidden planner material: ${identity}`);
  }
}

function addInertSemanticValidationMetadata(loaded: AceLoadedPackageManifest): AceLoadedPackageManifest {
  const validationTensors = directSemanticValidationTensors as unknown as Readonly<Record<string, AcePackageTensorRecord>>;
  const entries = Object.entries(validationTensors);
  if (entries.length !== DIRECT_SEMANTIC_VALIDATION_TENSOR_COUNT) {
    throw new Error("ACE direct-only semantic validation metadata has the wrong tensor count");
  }
  ...
  for (const [name, tensor] of entries) {
    ...
    if (tensor.phase !== "semantic" || Object.hasOwn(tensors, name) || fileNames.has(tensor.shard) || !sourceKeys.has(sourceKey)) {
      throw new Error(`ACE direct-only semantic validation metadata is invalid: ${name}`);
    }
...
function addUnusedPlannerTokenizerAliases(acquired: AceAcquiredModelFiles): AceAcquiredModelFiles {
  const forbidden = [...acquired.files.keys()].find((name) => FORBIDDEN_DIRECT_FILE.test(name));
  if (forbidden !== undefined) {
    throw new Error(`ACE direct-only acquisition unexpectedly retained ${forbidden}`);
  }
