...
export function loadWhisperEncoder(ctx, bin, man) {
  ...
  return {
    ...
    conv1w: g("conv1w").slice(),
    conv1b: g("conv1b").slice(),
    conv2w: g("conv2w").slice(),
    conv2b: g("conv2b").slice(),
    posw: ctx.upload(g("posw").slice(), TENC, D),
    lnf: [vec("lnf_w"), vec("lnf_b")],
    layers,
  };
}
...
export function whisperEncode(ctx, enc, mel) {
  ...
  const w1 = ctx.upload(enc.conv1w, 512, 80 * 3),
    b1 = ctx.upload(enc.conv1b, 1, 512);
  const w2 = ctx.upload(enc.conv2w, 512, 512 * 3),
    b2 = ctx.upload(enc.conv2b, 1, 512);
