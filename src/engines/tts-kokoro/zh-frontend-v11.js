...
function strictSplit(syl) {
  ...
  for (const i of INITIALS) {
    if (syl.startsWith(i)) { initial = i; break; }
  }
  ...
  if (initial) {
    ...
    const r = syl.slice(1);
    if (r.startsWith("u")) fin = r === "u" ? "v" : "v" + r.slice(1); // yu/yue/yuan/yun → v/ve/van/vn
    else if (r.startsWith("i")) fin = r;                              // yi/yin/ying → i/in/ing
...
export function chineseToZh11(text) {
  ...
  for (let i = 0; i < sylls.length; i++) {
    ...
    if (raw.length === 1 && (PUNC.has(raw) || PUNC_MAP[raw])) { out += PUNC_MAP[raw] ?? raw; continue; }
