...
// Shared demo-page core. Each page (stt.ts / tts.ts / analyze.ts) is a thin
// entry that calls initPlayground() with its registry category and UI flags.
// Engines implement a common interface (core/types), so the UI treats them
// uniformly: pick → load → run. Audio engines take a file; TTS takes text.
...
export interface PlaygroundOptions {
  /** Registry entries shown in the picker (filtered by EngineEntry.category). */
  category: EngineCategory;
  /** Initially selected engine; defaults to the category's first entry. */
  defaultEngineId?: string;
  /** 🎤 live-mic mode — wires the #mic button (STT page). */
  mic?: boolean;
  /** SRT/VTT caption downloads after file runs (STT page). */
  captions?: boolean;
  /** Custom-vocabulary + ITN row for engines exposing setVocabulary (STT page). */
  vocab?: boolean;
}
...
export function initPlayground(opts: PlaygroundOptions) {
  initSiteTheme();

  // Engine catalog lives in engines/registry.ts — a new engine registered
  // there with this page's category appears here automatically.
  const PAGE_ENGINES = ENGINES.filter((e) => e.category === opts.category);
  const ENTRIES: Record<string, EngineEntry> = Object.fromEntries(PAGE_ENGINES.map((e) => [e.id, { ...e, label: `${e.label} ✅` }]));
  const defaultEngineId = opts.defaultEngineId ?? PAGE_ENGINES[0].id;
  ...
  engineSel.value = defaultEngineId; // landing default (STT: the ASR demo, not the first registry entry)
  ...
  void Promise.all(
    PAGE_ENGINES.filter((e) => e.available).map(async (e) => {
      if (await e.available!().catch(() => false)) return;
      engineSel.querySelector(`option[value="${e.id}"]`)?.remove();
      delete ENTRIES[e.id];
      if (engineSel.value === "") engineSel.value = defaultEngineId;
    }),
  );
  ...
  function syncInputs() {
    ...
    if (audioInput) audioInput.hidden = kind !== "audio";
    const textInput = $("textInput");
    if (textInput) textInput.hidden = kind !== "text";
    // Custom-vocabulary box: Parakeet only (BK-tree rescorer).
    const vocabInput = $("vocabInput");
    if (vocabInput) vocabInput.hidden = !opts.vocab || engineSel.value !== "asr-parakeet";
  ...
  async function runAudioEngine(eng: Engine, audio: { samples: Float32Array; sampleRate: number }): Promise<string> {
    ...
    if (typeof any.transcribe === "function") {
      ...
      if (typeof any.setVocabulary === "function") {
        const raw = ($("vocab") as HTMLInputElement | null)?.value.trim() ?? "";
        any.setVocabulary(
          raw
            ? raw
                .split(",")
