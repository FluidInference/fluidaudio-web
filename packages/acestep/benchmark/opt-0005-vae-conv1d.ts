...
export const ACE_OPT_0005_VAE_CONV1D_KERNEL_SIZE = 7;
export const ACE_OPT_0005_VAE_CONV1D_SUPPORTED_DILATIONS =
  Object.freeze([1, 3, 9] as const);
export const ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK = 64;
export const ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES = 16;
export const ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS = 8;
export const ACE_OPT_0005_VAE_CONV1D_INPUT_TILE_STRIDE = 17;
export const ACE_OPT_0005_VAE_CONV1D_WEIGHT_TILE_STRIDE = 65;
export const ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE_X = 16;
export const ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE_Y = 8;
export const ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE = 128;
...
export interface AceOpt0005VaeConv1dOutputRange {
...
export interface AceOpt0005VaeConv1dPlan extends AceVaeConv1dShape {
  readonly outputFrames: number;
  readonly inputElements: number;
  readonly weightElements: number;
  readonly outputElements: number;
  readonly inputChannelChunk: typeof ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK;
  readonly inputChannelChunkCount: number;
  readonly tileFrames: typeof ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES;
  readonly tileChannels: typeof ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS;
  readonly inputTileStride: typeof ACE_OPT_0005_VAE_CONV1D_INPUT_TILE_STRIDE;
  readonly weightTileStride: typeof ACE_OPT_0005_VAE_CONV1D_WEIGHT_TILE_STRIDE;
  readonly workgroupSizeX: typeof ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE_X;
  readonly workgroupSizeY: typeof ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE_Y;
  readonly workgroupSize: typeof ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE;
  ...
  readonly outputRanges: readonly AceOpt0005VaeConv1dOutputRange[];
}

export interface AceOpt0005VaeConv1dDispatch {
  readonly label: string;
  readonly plan: AceOpt0005VaeConv1dPlan;
  readonly rangeCount: number;
  encodeRange(pass: GPUComputePassEncoder, rangeIndex: number): void;
  encode(pass: GPUComputePassEncoder): void;
}

interface CompiledAceOpt0005VaeConv1d {
...
/** Benchmark-only exact-loop, 64-input-channel-chunk K7 Conv1D candidate. */
export class AceOpt0005VaeConv1dChannelChunksKernel {
  private readonly compiled = new Map<
    string,
    Promise<CompiledAceOpt0005VaeConv1d>
  >();
  ...
  static create(device: GPUDevice): AceOpt0005VaeConv1dChannelChunksKernel {
    if (
      ...
      throw new Error(
        "OPT-0005 VAE Conv1D requires a 16x8 (128-lane) compute workgroup",
      );
    }
    return new AceOpt0005VaeConv1dChannelChunksKernel(device);
  }
  ...
  ): Promise<AceOpt0005VaeConv1dDispatch> {
    this.requireLive();
      ...
      encodeRange(pass: GPUComputePassEncoder, rangeIndex: number): void {
        ...
        if (range === undefined) {
          throw new RangeError(
            `${label} OPT-0005 range ${rangeIndex} is outside [0, ${plan.outputRangeCount})`,
          );
        }
  ...
  private pipelineFor(
    plan: AceOpt0005VaeConv1dPlan,
    hasBias: boolean,
  ): Promise<CompiledAceOpt0005VaeConv1d> {
    const key = convKey(plan, hasBias);
    const existing = this.compiled.get(key);
    if (existing !== undefined) return existing;
    const created = compileAceOpt0005VaeConv1d(this.device, plan, hasBias);
  ...
  private requireDeviceLimits(plan: AceOpt0005VaeConv1dPlan): void {
    if (plan.workgroupStorageBytes > this.device.limits.maxComputeWorkgroupStorageSize) {
      throw new RangeError(
        `OPT-0005 VAE Conv1D requires ${plan.workgroupStorageBytes} workgroup-storage bytes`,
      );
    }
    const maximumDispatch = this.device.limits.maxComputeWorkgroupsPerDimension;
    if (plan.outputRanges.some(({ workgroupsX, workgroupsY }) =>
      workgroupsX > maximumDispatch || workgroupsY > maximumDispatch
    )) {
      throw new RangeError("OPT-0005 VAE Conv1D exceeds the device dispatch dimension");
    }
    ...
    for (const [name, elements] of [
      ...
      if (checkedBytes(elements, name) > maximumBinding) {
        throw new RangeError(
          `OPT-0005 VAE Conv1D ${name} exceeds the device storage binding limit`,
        );
      }
    }
  }
  ...
  private requireLive(): void {
    if (this.destroyed) {
      throw new Error("OPT-0005 VAE Conv1D kernel was destroyed");
    }
  }
}

export function planAceOpt0005VaeConv1dChannelChunks(
  shape: AceVaeConv1dShape,
): AceOpt0005VaeConv1dPlan {
  const portable = planAceVaeConv1d(shape);
  if (shape.kernelSize !== ACE_OPT_0005_VAE_CONV1D_KERNEL_SIZE) {
    throw new RangeError("OPT-0005 VAE Conv1D requires kernel size 7");
  }
  if (
    shape.stride !== 1 ||
    !ACE_OPT_0005_VAE_CONV1D_SUPPORTED_DILATIONS.includes(
      shape.dilation as 1 | 3 | 9,
    )
  ) {
    throw new RangeError(
      "OPT-0005 VAE Conv1D requires stride 1 and dilation 1, 3, or 9",
    );
  }
  for (const [name, elements] of [
  ...
  const inputChannelChunkCount = Math.ceil(
    shape.inputChannels / ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK,
  );
  ...
  const workgroupsY = Math.ceil(
    shape.outputChannels / ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS,
  );
  if (workgroupsY > MAX_DISPATCH_DIMENSION) {
    throw new RangeError(
      "OPT-0005 VAE Conv1D output channels exceed the 2D dispatch domain",
    );
  }
  ...
  const maximumRowsPerRange = Math.min(
    ...
    Math.floor(
      ACE_VAE_DEFAULT_QUANTUM_WORK_POLICY.maximumConvolutionMultiplyAccumulates /
        multiplyAddsPerRow,
    ),
    MAX_DISPATCH_DIMENSION * ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES,
  );
  if (maximumRowsPerRange < 1) {
    throw new RangeError(
      "OPT-0005 VAE Conv1D cannot fit one complete-channel output row in a bounded range",
    );
  }

  const outputRanges: AceOpt0005VaeConv1dOutputRange[] = [];
  for (let batch = 0; batch < shape.batch; batch += 1) {
    for (
      ...
      const workgroupsX = Math.ceil(
        outputRowCount / ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES,
      );
      const lastStagedTime = firstOutputTime +
        (workgroupsX - 1) * ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES +
        (ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES - 1) +
        (shape.kernelSize - 1) * shape.dilation;
      requireWgslIndexable(lastStagedTime, "last staged input time");
      requireWgslIndexable(firstOutputTime + outputRowCount, "range end time");
  ...
  if (emittedOutputs !== portable.outputElements) {
    throw new Error("OPT-0005 VAE Conv1D range planner lost outputs");
  }
  ...
  return Object.freeze({
    ...
    inputChannelChunk: ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK,
    inputChannelChunkCount,
    tileFrames: ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES,
    tileChannels: ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS,
    inputTileStride: ACE_OPT_0005_VAE_CONV1D_INPUT_TILE_STRIDE,
    weightTileStride: ACE_OPT_0005_VAE_CONV1D_WEIGHT_TILE_STRIDE,
    workgroupSizeX: ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE_X,
    workgroupSizeY: ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE_Y,
    workgroupSize: ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE,
    inputTileElements,
    weightTileElements,
    inputTileBytes,
    weightTileBytes,
...
export function aceOpt0005VaeConv1dChannelChunksWgsl(
  shape: AceVaeConv1dShape,
  hasBias: boolean,
): string {
  const plan = planAceOpt0005VaeConv1dChannelChunks(shape);
...
fn main(
  ...
  let tile_first_time =
    range_first_time + group.x * ${ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES}u;
  let output_time = tile_first_time + local.x;
  let output_channel =
    group.y * ${ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS}u + local.y;
  ...
  // Concatenating increasing 64-channel chunks preserves the scalar source
  // order exactly: K outer, then input channel 0..INPUT_CHANNELS-1.
  for (var kernel = 0u; kernel < 7u; kernel += 1u) {
    for (
      ...
      let chunk_first_channel =
        input_channel_chunk * ${ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK}u;
      for (
        var tile_index = lane;
        tile_index < ${ACE_OPT_0005_VAE_CONV1D_TILE_FRAMES * ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK}u;
        tile_index += ${ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE}u
      ) {
        let tile_time =
          tile_index / ${ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK}u;
        let chunk_channel =
          tile_index % ${ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK}u;
        let input_channel = chunk_first_channel + chunk_channel;
        let padded_time =
          tile_first_time + tile_time + kernel * DILATION;
        var value = 0.0;
        ...
        input_tile[
          chunk_channel * ${ACE_OPT_0005_VAE_CONV1D_INPUT_TILE_STRIDE}u +
          tile_time
        ] = value;
      }
      for (
        var tile_index = lane;
        tile_index < ${ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS * ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK}u;
        tile_index += ${ACE_OPT_0005_VAE_CONV1D_WORKGROUP_SIZE}u
      ) {
        ...
        let weight_output_channel =
          group.y * ${ACE_OPT_0005_VAE_CONV1D_TILE_CHANNELS}u +
          tile_output_channel;
        var value = 0.0;
        if (
        ...
        weight_tile[
          tile_output_channel * ${ACE_OPT_0005_VAE_CONV1D_WEIGHT_TILE_STRIDE}u +
          chunk_channel
        ] = value;
      }
      workgroupBarrier();
      ...
      if (output_active) {
        ...
        // Invalid padding skips the complete channel chunk's arithmetic.
        if (padded_time >= PADDING) {
          let input_time = padded_time - PADDING;
          if (input_time < INPUT_FRAMES) {
            let chunk_channel_count = min(
              ${ACE_OPT_0005_VAE_CONV1D_INPUT_CHANNEL_CHUNK}u,
              INPUT_CHANNELS - chunk_first_channel
            );
            let weight_base =
              local.y * ${ACE_OPT_0005_VAE_CONV1D_WEIGHT_TILE_STRIDE}u;
            for (
...
async function compileAceOpt0005VaeConv1d(
  device: GPUDevice,
  plan: AceOpt0005VaeConv1dPlan,
  hasBias: boolean,
): Promise<CompiledAceOpt0005VaeConv1d> {
  const label = `ace-opt-0005-vae-conv1d-${convKey(plan, hasBias)}`;
  const module = device.createShaderModule({
    label,
    code: aceOpt0005VaeConv1dChannelChunksWgsl(plan, hasBias),
  });
  const compilation = await module.getCompilationInfo();
  const shaderErrors = compilation.messages.filter(({ type }) => type === "error");
  if (shaderErrors.length > 0) {
    throw new Error(
      `OPT-0005 VAE Conv1D WGSL failed: ${shaderErrors.map((message) =>
  ...
  if (
    ...
    throw new Error("OPT-0005 device reported an invalid uniform alignment");
  }
  ...
  if (
    parameterBytes > Number(device.limits.maxBufferSize) ||
    maximumDynamicOffset > MAX_WGSL_U32
  ) {
    throw new RangeError(
      "OPT-0005 VAE Conv1D range controls exceed the device buffer limit",
    );
  }
...
function checkedProduct(values: readonly number[], label: string): number {
  ...
  for (const value of values) {
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new RangeError(
        `OPT-0005 VAE Conv1D ${label} operand is not a non-negative safe integer`,
      );
    }
    product *= value;
    if (!Number.isSafeInteger(product)) {
      throw new RangeError(`OPT-0005 VAE Conv1D ${label} is not a safe integer`);
    }
  }
  return product;
}
...
function requireWgslIndexable(value: number, label: string): void {
  if (value > MAX_WGSL_U32) {
    throw new RangeError(
      `OPT-0005 VAE Conv1D ${label} exceeds WGSL's u32 indexing domain`,
    );
  }
}

// Stable short names used by the paired browser harness. The descriptive
// implementation names remain useful when comparing OPT-0005 with OPT-0004.
export {
  AceOpt0005VaeConv1dChannelChunksKernel as AceOpt0005VaeConv1dKernel,
  aceOpt0005VaeConv1dChannelChunksWgsl as aceOpt0005VaeConv1dWgsl,
  planAceOpt0005VaeConv1dChannelChunks as planAceOpt0005VaeConv1d,
};
