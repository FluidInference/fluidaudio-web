...
declare global {
  interface Window {
    __ACE_OPT0040_RESULT__?: Readonly<Record<string, unknown>>;
  }
}
...
prepareButton.addEventListener("click", () => {
  ...
  try {
    ...
    progress.textContent =
      "authenticating revision-6 package and running untimed exactness/determinism";
    const active = new Worker(
      new URL(
        "./opt-0040-vae-convtranspose-shape-selector-worker.ts",
        import.meta.url,
      ),
      { type: "module", name: "ace-opt-0040-convtranspose-selector" },
    );
    worker = active;
    active.addEventListener("message", (event: MessageEvent<WorkerMessage>) => {
      ...
      if (message.type === "ready-for-thermal-gate") {
        ...
        progress.textContent =
          "READY — wait 30 seconds, check notifyutil once, enter level 0, then run balanced timing";
      ...
      publish(Object.freeze({
        schema: "ace-opt-0040-page-failure-v1",
        experimentId: "OPT-0040",
        status: "failed",
        error: message.error,
      }), "failed");
      active.terminate();
...
runButton.addEventListener("click", () => {
  ...
  try {
    const now = Date.now();
    const gate = parseOpt0040ThermalGate(
    ...
    progress.textContent =
      "running control / selector / selector / control C512 timing, then cancellation and cleanup";
...
function publish(
  ...
  window.__ACE_OPT0040_RESULT__ = receipt;
  document.body.dataset.status = state;
  progress.textContent = state === "complete"
    ? "COMPLETE — exact balanced OPT-0040 receipt is ready"
...
function requireElement<ElementType extends Element>(selector: string): ElementType {
  ...
  if (element === null) throw new Error(`Missing OPT-0040 element ${selector}`);
  return element;
}
