...
import {
  ...
  type Opt0057RawComparison,
  type Opt0057ThermalGate,
  type Opt0057TimedArm,
  type Opt0057TimingSample,
  type Opt0057WaveformMetrics,
} from "./opt-0057-vae-k7-k4-row-reuse-c512-contract.js";

export const OPT_0057_BROWSER_SCHEMA =
  "ace-opt-0057-vae-revision7-k4-selector-c512-abba-v1" as const;
export const OPT_0057_C512_FRAMES = 512 as const;
export const OPT_0057_C512_INPUT_ELEMENTS = 32_768 as const;
export const OPT_0057_C512_OUTPUT_ELEMENTS = 1_966_080 as const;
export const OPT_0057_C512_OUTPUT_U16_WORDS = 3_932_160 as const;
export const OPT_0057_C512_FIXTURE_SHA256 =
  "eff0005ae48353fbc0a9ec86a5b2824b49e6fff6e899ea89af7d1c6e5870e899" as const;
export const OPT_0057_REV6_SCALAR_OUTPUT_SHA256 =
...
const EXPERIMENT_ID = "OPT-0057" as const;
...
type WorkerCommand =
  ...
  | Readonly<{
      readonly type: "run";
      readonly thermalGate: Opt0057ThermalGate;
    }>
...
interface OwnerTracker {
  active: Opt0057OwnedArm | null;
  liveOwners: number;
  peakLiveOwners: number;
  created: Record<Opt0057OwnedArm, number>;
  destroyed: Record<Opt0057OwnedArm, number>;
...
class RunObserver {
  ...
  begin(label: string): void {
    if (this.active !== undefined) {
      throw new Error("OPT-0057 observer already owns a run");
    }
    this.active = {
      label,
  ...
  finish(label: string): ObserverSummary {
    ...
    if (
      ...
      throw new Error(`OPT-0057 ${label} observation is incomplete`);
    }
...
globalThis.addEventListener("message", (event: MessageEvent<WorkerCommand>) => {
  operation = operation.then(async () => {
    ...
    if (command.type === "initialize") {
      if (prepared !== undefined) throw new Error("OPT-0057 is already prepared");
    ...
    if (command.type === "dispose") {
      ...
      await retained?.destroy(new DOMException(
        "OPT-0057 page disposed",
        "AbortError",
      ));
      return;
    }
    if (prepared === undefined) {
      throw new Error("OPT-0057 timing requested before READY");
    }
...
async function prepareGate(identity: Opt0018RunIdentity): Promise<PreparedGate> {
  ...
  const fixtureBytes = createAceOpt0011LatentFixture(OPT_0057_C512_FRAMES);
  if (
    fixtureBytes.byteLength !== OPT_0057_C512_INPUT_ELEMENTS * FLOAT32_BYTES ||
    await sha256Hex(fixtureBytes) !== OPT_0057_C512_FIXTURE_SHA256
  ) {
    throw new Error("OPT-0057 C512 fixture identity changed");
  }
  const fixture = new Float32Array(OPT_0057_C512_INPUT_ELEMENTS);
  fixture.set(new Float32Array(
    fixtureBytes.buffer,
    fixtureBytes.byteOffset,
    OPT_0057_C512_INPUT_ELEMENTS,
  ));
  ...
  try {
    ...
    const plan = planAceVaeChunkedDecode(OPT_0057_C512_FRAMES, {
      chunkFrames: OPT_0057_C512_FRAMES,
      overlapFrames: 64,
    });
    const window = requireOneC512Window(plan);
    const executions: ExecutionResult[] = [];
    for (const [index, arm] of OPT_0057_CORRECTNESS_ORDER.entries()) {
      postProgress(
        `untimed sequential correctness ${index + 1}/` +
          `${OPT_0057_CORRECTNESS_ORDER.length}: ${arm}`,
      );
    ...
    const k4OracleRuns = executions.filter((execution) =>
      execution.arm === "rev6-k4-oracle"
    );
    const candidateRuns = executions.filter((execution) =>
      execution.arm === "rev7-candidate"
    );
    if (
      scalarRuns.length !== 2 || k4OracleRuns.length !== 2 ||
      candidateRuns.length !== 2
    ) {
      throw new Error("OPT-0057 correctness ownership order changed");
    }
    const outputHashes = await Promise.all(executions.map((execution) =>
      sha256Float32(execution.output)
    ));
    const scalarDeterminism = compareOpt0057Raw(
      scalarRuns[0]!.output,
      scalarRuns[1]!.output,
    );
    const k4OracleDeterminism = compareOpt0057Raw(
      k4OracleRuns[0]!.output,
      k4OracleRuns[1]!.output,
    );
    const candidateDeterminism = compareOpt0057Raw(
      candidateRuns[0]!.output,
      candidateRuns[1]!.output,
    );
    const candidateToK4Oracle = Object.freeze([
      compareOpt0057Raw(k4OracleRuns[0]!.output, candidateRuns[0]!.output),
      compareOpt0057Raw(k4OracleRuns[1]!.output, candidateRuns[1]!.output),
    ]);
    ...
    const scalarHashesAccepted = scalarRuns.every((run) =>
      outputHashes[executions.indexOf(run)] ===
        OPT_0057_REV6_SCALAR_OUTPUT_SHA256
    );
    const rawOraclePassed = candidateToK4Oracle.every(rawComparisonPassed);
    const numericalPassed = numericalEnvelope.every(allMetricsPassed);
    const deterministic = rawComparisonPassed(scalarDeterminism) &&
      rawComparisonPassed(k4OracleDeterminism) &&
    ...
    if (
      ...
      throw new Error(
        "OPT-0057 sequential C512 correctness gate failed: " +
          JSON.stringify({
    ...
    const preparationReceipt = Object.freeze({
      schema: OPT_0057_BROWSER_SCHEMA,
      status: "ready",
      experimentId: EXPERIMENT_ID,
      identity,
      fixture: Object.freeze({
        frames: OPT_0057_C512_FRAMES,
        elements: OPT_0057_C512_INPUT_ELEMENTS,
        byteLength: fixtureBytes.byteLength,
        sha256: OPT_0057_C512_FIXTURE_SHA256,
      }),
      ...
      topology: Object.freeze({
        ...
        nativeLayoutK4Oracle: k4OracleRuns[0]!.topologySummary,
      ...
      correctness: Object.freeze({
        order: OPT_0057_CORRECTNESS_ORDER,
...
async function runTimedGate(
  ...
  thermalGate: Opt0057ThermalGate,
): Promise<Readonly<Record<string, unknown>>> {
  const executions: ExecutionResult[] = [];
  for (const [index, arm] of OPT_0057_TIMED_ORDER.entries()) {
    ...
    if (
      ...
      throw new Error("OPT-0057 timed topology changed after READY");
    }
  ...
  const outputReceipts = await Promise.all(executions.map(async (
    ...
    return Object.freeze({
      ...
      comparisonToUntimedReference: compareOpt0057Raw(
        reference,
        execution.output,
      ),
  ...
  const timingSamples: Opt0057TimingSample[] = executions.map((execution) =>
    Object.freeze({
      arm: execution.arm as Opt0057TimedArm,
  ...
  const performance = evaluateOpt0057BalancedTiming(timingSamples);
  ...
  return Object.freeze({
    ...
    status: passed
      ...
      : correctnessAndLifecyclePassed
        ...
        : "failed",
    decision: passed
      ? "positive-authenticated-revision7-c512-gate-passed"
      : correctnessAndLifecyclePassed
        ? "negative-below-k7-or-decoder-speed-gate"
    ...
    protocol: Object.freeze({
      correctnessOrder: OPT_0057_CORRECTNESS_ORDER,
      timedOrder: OPT_0057_TIMED_ORDER,
...
async function withOwnedBackend<Value>(
  ...
  if (tracker.active !== null || tracker.liveOwners !== 0) {
    throw new Error(
      `OPT-0057 refused co-resident ${arm}; ${String(tracker.active)} is live`,
    );
  }
  tracker.active = arm;
  ...
  try {
    ...
    phase = undefined;
    backend = arm === "rev6-k4-oracle"
      ? await createNativeLayoutK4OracleBackend(
          context.device,
          pkg,
          plan,
          fixture,
    ...
    try {
      if (backend !== undefined) {
        const reason = new DOMException(
          `OPT-0057 ${arm} sequential owner complete`,
          "AbortError",
        );
  ...
  if (!useCompleted) {
    throw new Error(`OPT-0057 ${arm} owner did not complete`);
  }
  return Object.freeze({
    value: value!,
    lifecycle: Object.freeze({
...
async function createStandardBackend(
  ...
  arm: Exclude<Opt0057OwnedArm, "rev6-k4-oracle">,
  observer: RunObserver,
  ...
  const common = {
    ...
    maximumWindowFrames: OPT_0057_C512_FRAMES,
...
/**
 * Benchmark-private full-decoder oracle: revision-6 native K7 weights, the
 * exact OPT-0040 ConvTranspose selector, and OPT-0024 K4 arithmetic only for
 * the twelve labels selected by OPT-0057. It is never a runtime fallback.
 */
async function createNativeLayoutK4OracleBackend(
  device: GPUDevice,
  pkg: PreparedPackage,
  plan: AceVaeChunkedDecodePlan,
  fixture: Float32Array<ArrayBuffer>,
  ...
  try {
    ...
    const dispatchSet = await runtime.createChunkDispatchSet(
      "opt-0057-c512-native-layout-opt-0024-k4-oracle",
      plan.latentFrames,
      OPT_0057_C512_FRAMES,
...
function operationLabelFromDispatchLabel(label: string): string {
  ...
  if (route === undefined) {
    throw new Error(`OPT-0057 could not authenticate K7 dispatch label ${label}`);
  }
...
function createOracleBuffers(
  ...
  const graph = planAceVaeDecoder(OPT_0057_C512_FRAMES);
...
async function executeBackend(
  ...
  try {
    ...
    if (
      output.length !== OPT_0057_C512_OUTPUT_ELEMENTS ||
      profile.windowIndex !== window.index ||
      profile.inputFrames !== OPT_0057_C512_FRAMES ||
      ...
      throw new Error(`OPT-0057 ${label} scheduling topology changed`);
    }
    const scan = scanOutput(output);
    if (
      scan["elementCount"] !== OPT_0057_C512_OUTPUT_ELEMENTS ||
      scan["nonFiniteCount"] !== 0 || scan["nonzeroCount"] === 0 ||
      scan["stereoDifferenceFrameCount"] === 0 ||
      scan["firstFrameFinite"] !== 1 || scan["lastFrameFinite"] !== 1
    ) {
      throw new Error(`OPT-0057 ${label} output is incomplete`);
    }
    ...
    if (
      ...
      throw new Error(`OPT-0057 ${label} family timing is incomplete`);
    }
    return Object.freeze({
...
function validateTopology(
  ...
  const scalarProfile = arm !== "rev7-candidate";
  const expectedProfile = scalarProfile
    ? ACE_OPT_0040_VAE_FP16_FIXED32_SHAPE_SELECTED_PROFILE
    : ACE_OPT_0054_VAE_FP16_FIXED32_REVISION7_PROFILE;
  const expectedKernelTopology = scalarProfile
    ? ACE_OPT_0040_VAE_FP16_FIXED32_SHAPE_SELECTED_KERNEL_TOPOLOGY
    : ACE_OPT_0054_VAE_FP16_FIXED32_REVISION7_KERNEL_TOPOLOGY;
  const expectedK7SelectedKernel = arm === "rev6-k4-oracle"
    ? ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_KERNEL_ID
    : arm === "rev7-candidate"
      ? ACE_OPT_0051_VAE_CONV1D_ROW_REUSE_16X64_KERNEL_ID
      : null;
  ...
  if (
    ...
    topology.uniqueWindowFrames[0] !== OPT_0057_C512_FRAMES ||
    window === undefined || window.inputFrames !== OPT_0057_C512_FRAMES ||
    ...
    throw new Error(`OPT-0057 ${arm} complete C512 kernel topology changed`);
  }
  ...
  for (const entry of k7Operations) {
    ...
    if (
      ...
      throw new Error(`OPT-0057 ${arm} K7 route changed at ${entry.operationLabel}`);
    }
    if (selected) {
  ...
  const expectedTranspose = arm === "rev7-candidate"
    ? [
  ...
  if (
    ...
    throw new Error(`OPT-0057 ${arm} operation-level routing changed`);
  }
  ...
  if (
    ...
    throw new Error(`OPT-0057 ${arm} changed an unrelated operation owner`);
  }
  return Object.freeze({
    arm,
    runtimeProfileId: backend.runtimeProfileId,
    kernelSetId: backend.kernelSetId,
    runtimeProfileRole: arm === "rev6-k4-oracle"
      ? "revision6-package-binding-and-non-k7-owner-base"
      : "authenticated-runtime-profile",
    effectiveK7Arithmetic: arm === "rev6-scalar"
      ? "scalar-fp32"
      : "selected-label-opt-0024-fp16-dot4-then-fp32-running-state",
    ...
    benchmarkPrivateArithmeticOverride: arm === "rev6-k4-oracle",
  });
}
...
async function authenticatePackage(
  ...
  if (
    ...
    throw new Error(
      `OPT-0057 authenticated revision-${revision} package identity changed`,
    );
  }
  return Object.freeze({
    revision,
...
async function acquirePackageFiles(
  ...
  if (
    ...
    throw new Error(
      `OPT-0057 revision-${pkg.revision} package acquisition changed`,
    );
  }
...
async function loadVaePhase(
  ...
  if (phase.residentBytes !== ACE_EXPERIMENTAL_VAE_PARAMETER_BYTES) {
    phase.destroy();
    throw new Error(`OPT-0057 ${arm} resident VAE bytes changed`);
  }
  return phase;
}
...
function requireOneC512Window(plan: AceVaeChunkedDecodePlan): AceVaeDecodeWindow {
  ...
  if (
    plan.latentFrames !== OPT_0057_C512_FRAMES ||
    plan.maximumWindowFrames !== OPT_0057_C512_FRAMES ||
    plan.windows.length !== 1 || window === undefined || window.index !== 0 ||
    window.latentWindowFrames !== OPT_0057_C512_FRAMES ||
    window.discardPrefixLatentFrames !== 0 ||
    window.discardSuffixLatentFrames !== 0
  ) {
    throw new Error("OPT-0057 plan is not one exact C512 window");
  }
  return window;
}
...
): Readonly<Record<"joint" | "left" | "right", Opt0057WaveformMetrics>> {
  return Object.freeze({
    joint: compareOpt0057Waveforms(control, candidate),
    left: compareOpt0057Waveforms(control, candidate, 2, 0),
    right: compareOpt0057Waveforms(control, candidate, 2, 1),
  });
}

function allMetricsPassed(
  metrics: Readonly<Record<"joint" | "left" | "right",
    Opt0057WaveformMetrics>>,
): boolean {
  return metrics.joint.passed && metrics.left.passed && metrics.right.passed;
}

function rawComparisonPassed(comparison: Opt0057RawComparison): boolean {
  return comparison.comparedU32WordCount === OPT_0057_C512_OUTPUT_ELEMENTS &&
    comparison.comparedU16WordCount === OPT_0057_C512_OUTPUT_U16_WORDS &&
...
function requireFixed32Subgroups(context: AceWebGpuDeviceContext): void {
  ...
  if (
    ...
    throw new Error("OPT-0057 requires stock fixed 32-lane WebGPU subgroups");
  }
}

function validateThermalGate(
  gate: Opt0057ThermalGate,
  ...
  if (
    ...
    throw new Error("OPT-0057 worker rejected the one-check thermal gate");
  }
}
