...
declare global {
  interface Window {
    __ACE_OPT0043_RESULT__?: Readonly<Record<string, unknown>>;
  }
}
...
const EXPERIMENT_ID = "OPT-0043" as const;
const RECEIPT_SCHEMA = "ace-opt-0043-webgpu-timestamp-profile-v1";
const ROWS = 2_250;
...
void prepareHarness().then(
  (prepared) => {
    ...
    progress.textContent =
      "READY — exact OPT-0032 fixtures passed; GPU timestamp timing has not run";
...
async function prepareHarness(): Promise<PreparedHarness> {
  ...
  const device = await adapter.requestDevice({
    label: "ace-opt-0043-webgpu-timestamp-profile-device",
...
async function runTiming(prepared: PreparedHarness): Promise<void> {
  ...
  for (const [roundIndex, round] of TIMING_ROUNDS.entries()) {
    for (const shapeIndex of round.shapeOrder) {
      ...
      if (shape === undefined) throw new Error("OPT-0043 timing shape changed");
  ...
  if (prepared.uncapturedErrors.length !== 0) {
    throw new Error("OPT-0043 observed an uncaptured timing GPU error");
  }
  const timingInputs: TimingInput[] = prepared.shapes.map(({ spec }) => {
    const entry = samples.get(spec.id);
    if (entry === undefined) throw new Error("OPT-0043 timing sample missing");
    return Object.freeze({
      id: spec.id,
  ...
  const runCompletedAtEpochMilliseconds = Date.now();
  const timing = summarizeOpt0043Timing(timingInputs);
  ...
  const receipt = Object.freeze({
    ...
    timing: Object.freeze({
      ...
      runCompletedAtEpochMilliseconds,
  ...
  progress.textContent = receipt.passed
    ? "completed — GPU timestamp and submit-to-drain profile captured"
    : "failed — invalid GPU timestamp profile";
}

export function summarizeOpt0043Timing(
  inputs: readonly TimingInput[],
): Readonly<Record<string, unknown>> {
  if (inputs.length !== FULL_SHAPES.length) {
    throw new Error("OPT-0043 timing requires all four production shapes");
  }
  ...
  const strata = inputs.map((input, index) => {
    ...
    if (spec === undefined || spec.id !== input.id) {
      throw new Error("OPT-0043 timing shape order changed");
    }
    const armSummaries = Object.fromEntries(
      (["control", "candidate"] as const).map((arm) => {
        const armSamples = input.samples[arm];
        requireTwoTimestampSamples(armSamples, `${spec.id} ${arm}`);
        const gpuMilliseconds = mean2(
          armSamples.map((sample) => sample.gpuMilliseconds),
          `${spec.id} ${arm} GPU`,
        );
        const wallMilliseconds = mean2(
        ...
        if (armSamples.some((sample) =>
          ...
          throw new Error(`OPT-0043 ${spec.id} ${arm} work changed between rounds`);
        }
  ...
  const weightedArms = Object.fromEntries(
    (["control", "candidate"] as const).map((arm) => [arm, Object.freeze({
  ...
  return Object.freeze({
    ...
    candidateSpeedup: Object.freeze({
      gpu: weightedGpuMilliseconds.control /
        weightedGpuMilliseconds.candidate,
      wall: weightedWallMilliseconds.control /
        weightedWallMilliseconds.candidate,
    }),
    passed: true,
  });
}
...
function requireAdapter(adapter: GPUAdapter): void {
  ...
  if (!adapter.features.has("shader-f16") ||
    ...
    throw new Error(
      "OPT-0043 requires stock Chrome timestamp-query, shader-f16, fixed32 subgroups, WG128, and full-shape storage limits",
    );
  }
}
...
async function timeDispatch(
  ...
  if (timestampReadback.mapState !== "unmapped") {
    throw new Error("OPT-0043 timestamp readback must be unmapped before timing");
  }
  const encoder = device.createCommandEncoder({
    label: `opt0043-${spec.id}-${arm}-timestamp-sample`,
  });
  const pass = encoder.beginComputePass({
    label: `opt0043-${spec.id}-${arm}-timestamped-compute`,
    timestampWrites: {
  ...
  if (timestampEnd <= timestampBegin) {
    throw new Error(
      `OPT-0043 ${spec.id} ${arm} returned a non-positive timestamp interval`,
    );
  }
  ...
  if (!Number.isSafeInteger(gpuElapsedNanoseconds) ||
    ...
    throw new Error(`OPT-0043 ${spec.id} ${arm} returned invalid timing`);
  }
  const plan = arm === "control"
...
function mean2(samples: readonly number[], label: string): number {
  if (samples.length !== 2 ||
    ...
    throw new Error(`OPT-0043 ${label} requires two finite positive samples`);
  }
  return (samples[0]! + samples[1]!) / 2;
}

function requireTwoTimestampSamples(
  ...
  if (samples.length !== 2 || samples.some((sample) =>
    ...
    throw new Error(
      `OPT-0043 ${label} requires two finite positive timestamp samples`,
    );
  }
}
...
function tflops(multiplyAdds: number, milliseconds: number): number {
  if (!Number.isSafeInteger(multiplyAdds) || multiplyAdds <= 0 ||
    ...
    throw new Error("OPT-0043 TFLOP/s inputs must be finite and positive");
  }
...
function requireLittleEndianHost(): void {
  ...
  if (new Uint8Array(words.buffer)[0] !== 0x02) {
    throw new Error("OPT-0043 fixtures require a little-endian host");
  }
}
...
function publish(
  ...
  window.__ACE_OPT0043_RESULT__ = receipt;
