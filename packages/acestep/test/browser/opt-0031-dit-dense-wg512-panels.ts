...
import candidateCoreSource from
  "../../src/webgpu/kernels/dit-dense-fp16-wg512-panels.ts?raw";
import {
...
declare global {
  interface Window {
    __ACE_OPT0031_RESULT__?: Readonly<Record<string, unknown>>;
  }
}

export type Opt0031Arm = "current" | "candidate";

export interface Opt0031ShapeSpec {
  readonly id: "h-h" | "h-1024" | "h-6144" | "6144-h";
  readonly shape: AceGemmShape;
  readonly productionMultiplicity: 4 | 2 | 1;
}

export interface Opt0031TimingInput {
  readonly id: Opt0031ShapeSpec["id"];
  readonly samples: Readonly<Record<Opt0031Arm, readonly number[]>>;
}

export interface Opt0031ThermalGate {
...
interface PreparedShape {
  readonly spec: Opt0031ShapeSpec;
  readonly dispatches: Readonly<Record<Opt0031Arm, AceGemmDispatch>>;
...
interface PreparedHarness {
  ...
  readonly candidateKernel: AceOpt0031DenseWg512PanelsKernel;
...
const EXPERIMENT_ID = "OPT-0031" as const;
const ROWS = 2_250;
const HIDDEN = 2_048;
...
const OUTPUT_PREFILL_QNAN_U32 = 0x7fc0_3155;
...
// Browser-control round trips in Codex can exceed five seconds even when the
// click follows immediately. Keep the receipt explicit, but do not reject an
// otherwise nominal one-shot gate for tooling latency alone.
...
const SHAPE_SPECS = Object.freeze([
  shapeSpec("h-h", HIDDEN, HIDDEN, 4),
  shapeSpec("h-1024", HIDDEN, 1_024, 2),
  shapeSpec("h-6144", HIDDEN, EXPANDED, 2),
  shapeSpec("6144-h", EXPANDED, HIDDEN, 1),
]);

export function buildOpt0031ShapeSpecs(): readonly Opt0031ShapeSpec[] {
  return SHAPE_SPECS;
}

export function buildOpt0031TimingOrders(): readonly Readonly<{
  roundIndex: number;
  shapeIndex: number;
  order: readonly Opt0031Arm[];
}>[] {
  return Object.freeze(Array.from({ length: TIMING_ROUNDS }, (_, roundIndex) => {
    ...
    return rotatedShapeOrder.map((shapeIndex) => Object.freeze({
      roundIndex,
      shapeIndex,
      order,
    }));
...
export function summarizeOpt0031Timing(
  inputs: readonly Opt0031TimingInput[],
): Readonly<Record<string, unknown>> {
  if (inputs.length !== SHAPE_SPECS.length) {
    throw new Error("OPT-0031 requires all four exact-shape timings");
  }
  ...
  const strata = inputs.map((input, index) => {
    ...
    if (spec === undefined || input.id !== spec.id) {
      throw new Error("OPT-0031 timing shape order changed");
    }
    const currentMedian = median4(input.samples.current);
    const candidateMedian = median4(input.samples.candidate);
    weightedCurrent += currentMedian * spec.productionMultiplicity;
    weightedCandidate += candidateMedian * spec.productionMultiplicity;
    return Object.freeze({
      ...
      candidateNoSlower: candidateMedian <= currentMedian,
      speedup: currentMedian / candidateMedian,
    });
  });
  const weightedSpeedup = weightedCurrent / weightedCandidate;
  const everyShapeNoSlower = strata.every(({ candidateNoSlower }) =>
    candidateNoSlower
  );
  const passed = everyShapeNoSlower &&
    weightedSpeedup >= REQUIRED_WEIGHTED_SPEEDUP;
  return Object.freeze({
    samplesPerArmPerShape: TIMING_ROUNDS,
    weighted: Object.freeze({
      multiplicities: "4/2/2/1",
      currentMilliseconds: weightedCurrent,
      candidateMilliseconds: weightedCandidate,
      savingMilliseconds: weightedCurrent - weightedCandidate,
      speedup: weightedSpeedup,
      requiredSpeedup: REQUIRED_WEIGHTED_SPEEDUP,
    }),
    everyShapeNoSlower,
...
export function parseOpt0031ThermalGate(
  ...
  if (
    ...
    throw new Error("OPT-0031 thermal gate is incomplete, stale, or non-nominal");
  }
  return Object.freeze({
    source: THERMAL_SOURCE,
    startedAtEpochMilliseconds,
...
function installBrowserHarness(): void {
  ...
  void prepareHarness((message) => {
    ...
    (value) => {
      ...
      progress.textContent =
        "READY — all four complete outputs are raw-U32 exact; timing has not run";
  ...
  run.addEventListener("click", () => {
    ...
    progress.textContent = "running four rotated balanced timing rounds";
    const owned = prepared;
    prepared = undefined;
    void runTimedGate(owned).then(
      (receipt) => finishPage("completed", receipt),
      (error: unknown) => {
        owned.destroy();
...
async function prepareHarness(
  ...
  const device = await adapter.requestDevice({
    label: "ace-opt-0031-dense-wg512-panels-device",
  ...
  const candidateKernel = AceOpt0031DenseWg512PanelsKernel.create(
    device,
    capability,
  );
  ...
  try {
    for (const [index, spec] of SHAPE_SPECS.entries()) {
      updateProgress(`compiling and checking full output ${index + 1}/4: ${spec.id}`);
    ...
    if (uncapturedErrors.length !== 0) {
      throw new Error(`OPT-0031 observed ${uncapturedErrors.length} GPU errors`);
    }
    const correctnessCases = shapes.map(({ correctness }) => correctness);
    return Object.freeze({
      adapter,
      device,
      tracker,
...
async function prepareShape(
  ...
  candidateKernel: AceOpt0031DenseWg512PanelsKernel,
  spec: Opt0031ShapeSpec,
  ordinal: number,
): Promise<PreparedShape> {
  const currentPlan = planAceOpt0009DenseGemm(spec.shape);
  const candidatePlan = planAceOpt0031DenseWg512Panels(spec.shape);
...
async function verifyCompleteShape(
  ...
  dispatches: Readonly<Record<Opt0031Arm, AceGemmDispatch>>,
  ...
  requireExactComparison(currentFirst.words, currentRerun.words,
  ...
  requireExactComparison(currentFirst.words, candidateFirst.words,
  ...
  requireExactComparison(currentFirst.words, candidateRerun.words,
    `${spec.id} candidate rerun`);
  requireExactComparison(candidateFirst.words, candidateRerun.words,
  ...
  return Object.freeze({
    ...
    mismatchCount: 0,
    outputSha256: await sha256U32(currentFirst.words),
    nonFiniteCount: 0,
    qNaNPrefillCount: 0,
...
async function executeCorrectnessDispatch(
  ...
  try {
    ...
    for (let index = 0; index < guardWords; index += 1) {
      ...
      suffixCanaryIntact &&= all[lastOutput + index] === STORAGE_GUARD_U32;
    }
...
async function runTimedGate(
  ...
  const thermal = parseOpt0031ThermalGate(
  ...
  if (prepared.uncapturedErrors.length !== 0) {
    throw new Error("OPT-0031 observed an uncaptured GPU error");
  }
  const timing = summarizeOpt0031Timing(timingInputs.map((input) =>
    Object.freeze({
      id: input.id,
  ...
  return Object.freeze({
    schema: "ace-opt-0031-dit-dense-wg512-panels-v1",
    status: "completed",
    ...
    protocol: Object.freeze({
      thermal,
      fullRawU32CorrectnessCompletedBeforeReady: true,
      bothKernelsCompiledBeforeReady: true,
      correctnessExecutionsServeAsUntimedWarmups: true,
      timingOrder:
        "shape order left-rotated by round; AB in rounds 1/3 and BA in rounds 2/4",
      authoritativeTiming:
    ...
    decision: Object.freeze({
      ...
      primitiveQualifierPassed: timing["passed"],
...
function createActivationBuffer(
  ...
  const buffer = tracker.create(device, {
    label: `opt-0031-${spec.id}-activation`,
    size: elements * 4,
...
function createWeightBuffer(
  ...
  const buffer = tracker.create(device, {
    label: `opt-0031-${spec.id}-packed-weight`,
    size: elements * 2,
    usage: GPUBufferUsage.STORAGE,
    mappedAtCreation: true,
  });
...
function createGuardedOutput(
  ...
  const prefill = tracker.create(device, {
    label: `opt-0031-${spec.id}-output-prefill`,
    size: totalBytes,
  ...
  prefill.unmap();
  const buffer = tracker.create(device, {
    label: `opt-0031-${spec.id}-guarded-output`,
    size: totalBytes,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST |
      GPUBufferUsage.COPY_SRC,
  });
  const readback = tracker.create(device, {
    label: `opt-0031-${spec.id}-readback`,
    size: totalBytes,
  ...
  for (const spec of SHAPE_SPECS) {
    generatedShaders.push(Object.freeze({
      ...
      candidateSha256: await sha256Text(
        aceOpt0031DenseWg512PanelsWgsl(spec.shape),
      ),
    }));
  }
  return Object.freeze({
    currentCoreSourceSha256: await sha256Text(currentCoreSource),
    candidateCoreSourceSha256: await sha256Text(candidateCoreSource),
    candidateKernelSetId: ACE_OPT_0031_DENSE_WG512_KERNEL_SET_ID,
...
function requireAdapter(adapter: GPUAdapter): void {
  ...
  if (
    ...
    adapter.limits.maxComputeWorkgroupSizeX < 512 ||
    ...
    throw new Error(
      "OPT-0031 requires fixed32 subgroups, WG512, 16896B shared storage, and full-shape buffers",
    );
  }
}
...
function shapeSpec(
  id: Opt0031ShapeSpec["id"],
  inner: number,
  columns: number,
  productionMultiplicity: Opt0031ShapeSpec["productionMultiplicity"],
): Opt0031ShapeSpec {
  return Object.freeze({
    id,
...
function median4(samples: readonly number[]): number {
  if (
    samples.length !== TIMING_ROUNDS ||
    samples.some((sample) => !Number.isFinite(sample) || sample <= 0)
  ) {
    throw new Error("OPT-0031 requires four finite positive samples per arm");
  }
...
function requiredParameter(parameters: URLSearchParams, name: string): string {
  ...
  if (value === null || value.length === 0) {
    throw new Error(`OPT-0031 thermal field ${name} is missing`);
  }
  return value;
}
...
function requiredFiniteParameter(
  ...
  if (!Number.isFinite(value)) {
    throw new Error(`OPT-0031 thermal field ${name} is not finite`);
  }
  return value;
}
...
function finishPage(
  ...
  window.__ACE_OPT0031_RESULT__ = receipt;
}
