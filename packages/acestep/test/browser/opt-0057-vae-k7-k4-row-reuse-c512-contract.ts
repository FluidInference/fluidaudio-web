export const OPT_0057_REQUIRED_K7_SPEEDUP = 1.50 as const;

export const OPT_0057_NUMERICAL_ENVELOPE = Object.freeze({
  nrmseMaximum: 0.003,
  snrMinimumDb: 50,
...
export interface Opt0057ThermalGate {
...
export interface Opt0057RawComparison {
...
export interface Opt0057WaveformMetrics {
...
export type Opt0057TimedArm = "rev6-scalar" | "rev7-candidate";

export interface Opt0057TimingSample {
  readonly arm: Opt0057TimedArm;
...
export function parseOpt0057ThermalGate(
  ...
  if (
    ...
    throw new Error(
      "OPT-0057 requires one truthful level-0 notifyutil check after a 30-second wait",
    );
  }
...
export function compareOpt0057Raw(
  control: Float32Array,
  candidate: Float32Array,
): Opt0057RawComparison {
  if (
    control.length !== candidate.length ||
    control.byteLength !== candidate.byteLength
  ) {
    throw new RangeError("OPT-0057 output lengths differ");
  }
...
export function compareOpt0057Waveforms(
  control: Float32Array,
  candidate: Float32Array,
  stride = 1,
  offset = 0,
  ...
  if (
    ...
    throw new RangeError("OPT-0057 waveform geometry is invalid");
  }
  let count = 0;
  let sumControl = 0;
  let sumCandidate = 0;
  ...
  if (count === 0) throw new RangeError("OPT-0057 waveform comparison is empty");
  ...
  const limits = OPT_0057_NUMERICAL_ENVELOPE;
...
export function evaluateOpt0057BalancedTiming(
  samples: readonly Opt0057TimingSample[],
  ...
  if (
    ...
    throw new Error("OPT-0057 timing samples are not balanced finite AB/BA");
  }
  ...
  const medianK7Passed = medianK7Speedup >= OPT_0057_REQUIRED_K7_SPEEDUP;
  ...
  return Object.freeze({
    ...
    aggregate: Object.freeze({
      medianK7Speedup,
      requiredMedianK7Speedup: OPT_0057_REQUIRED_K7_SPEEDUP,
      medianK7Passed,
      medianConvTransposeSpeedup:
        median(scalar.map((sample) =>
          sample.convTransposeFamilySubmitThroughDrainMs
        )) / median(candidate.map((sample) =>
          sample.convTransposeFamilySubmitThroughDrainMs
        )),
...
function evaluatePair(
  scalar: Opt0057TimingSample,
  candidate: Opt0057TimingSample,
  order: "AB" | "BA",
): Readonly<{
  order: "AB" | "BA";
  k7Speedup: number;
  ...
  return Object.freeze({
    order,
    k7Speedup,
    convTransposeSpeedup: scalar.convTransposeFamilySubmitThroughDrainMs /
      candidate.convTransposeFamilySubmitThroughDrainMs,
    decoderSpeedup,
    outerWindowSpeedup: scalar.outerWindowWallMs /
...
function median(values: readonly number[]): number {
  if (values.length === 0 || values.some((value) => !Number.isFinite(value))) {
    throw new RangeError("OPT-0057 median requires finite values");
  }
...
function requiredNumber(parameters: URLSearchParams, name: string): number {
  ...
  if (!Number.isFinite(value)) {
    throw new Error(`OPT-0057 requires finite ${name}`);
  }
  return value;
}
