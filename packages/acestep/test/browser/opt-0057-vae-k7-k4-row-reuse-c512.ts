...
import { parseOpt0057ThermalGate } from
  "./opt-0057-vae-k7-k4-row-reuse-c512-contract.js";

declare global {
  interface Window {
    __ACE_OPT0057_RESULT__?: Readonly<Record<string, unknown>>;
  }
}
...
prepareButton.addEventListener("click", () => {
  ...
  try {
    ...
    const active = new Worker(
      new URL(
        "./opt-0057-vae-k7-k4-row-reuse-c512-worker.ts",
        import.meta.url,
      ),
      { type: "module", name: "ace-opt-0057-revision7-c512" },
    );
    worker = active;
    active.addEventListener("message", (event: MessageEvent<WorkerMessage>) => {
      ...
      publish(Object.freeze({
        schema: "ace-opt-0057-page-failure-v1",
        experimentId: "OPT-0057",
        status: "failed",
        error: message.error,
      }), "failed");
      active.terminate();
...
runButton.addEventListener("click", () => {
  ...
  try {
    const now = Date.now();
    const gate = parseOpt0057ThermalGate(
...
function publish(
  ...
  window.__ACE_OPT0057_RESULT__ = receipt;
  document.body.dataset.status = state;
  progress.textContent = state === "complete"
    ? "COMPLETE — authenticated OPT-0057 receipt is ready"
...
function requireElement<ElementType extends Element>(selector: string): ElementType {
  ...
  if (element === null) throw new Error(`Missing OPT-0057 element ${selector}`);
  return element;
}
