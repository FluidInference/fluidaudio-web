...
declare global {
  interface Window {
    __ACE_OPT0034_RESULT__?: Readonly<Record<string, unknown>>;
  }
}
...
prepareButton.addEventListener("click", () => {
  ...
  try {
    ...
    progress.textContent = "initializing manifests, cache, and WebGPU device";
    const active = new Worker(
      new URL(
        "./opt-0034-dit-command-buffer-coalescing-worker.ts",
        import.meta.url,
      ),
      { type: "module", name: "ace-opt-0034-dit-coalescing" },
    );
    worker = active;
    active.addEventListener("message", (event: MessageEvent<WorkerMessage>) => {
      ...
      if (message.type === "ready-for-thermal-gate") {
        ...
        progress.textContent =
          "READY — wait 30 nominal seconds, record the thermal observation, then run once";
      ...
      publish(Object.freeze({
        schema: "ace-opt-0034-page-failure-v1",
        experimentId: "OPT-0034",
        status: "failed",
        error: message.error,
      }), "failed");
      active.terminate();
...
runButton.addEventListener("click", () => {
  ...
  try {
    const now = Date.now();
    const parameters = thermalParameters(now);
    const gate = parseOpt0034ThermalGate(
      parameters,
    ...
    progress.textContent =
      "running batch1, batch8, batch16; each arm stops after its detached final latent";
...
function publish(
  ...
  window.__ACE_OPT0034_RESULT__ = receipt;
  document.body.dataset.status = status;
  progress.textContent = status === "complete"
    ? "COMPLETE — compact exact A/B/C receipt is ready"
...
function requireElement<ElementType extends Element>(selector: string): ElementType {
  ...
  if (element === null) throw new Error(`Missing OPT-0034 element ${selector}`);
  return element;
}
