export interface Opt0040ThermalGate {
...
export function parseOpt0040ThermalGate(
  ...
  if (
    ...
    throw new Error(
      "OPT-0040 requires one truthful level-0 notifyutil check after a 30-second wait",
    );
  }
...
function requiredNumber(parameters: URLSearchParams, name: string): number {
  ...
  if (!Number.isFinite(value)) {
    throw new Error(`OPT-0040 requires finite ${name}`);
  }
  return value;
}
