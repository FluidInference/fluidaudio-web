...
import candidateKernelSource from
  "../../src/webgpu/kernels/dit-dense-fp16-weight-multicast.ts?raw";
...
declare global {
  interface Window {
    __ACE_OPT0078_RESULT__?: Readonly<Record<string, unknown>>;
  }
}

export type Opt0078Arm = "current" | "candidate";

export interface Opt0078ShapeSpec {
...
export interface Opt0078TimestampSample {
...
export interface Opt0078TimingInput {
  readonly id: Opt0078ShapeSpec["id"];
  readonly samples: Readonly<Record<
    Opt0078Arm,
    readonly Opt0078TimestampSample[]
  >>;
}

export interface Opt0078ThermalLaunchGate {
...
export interface Opt0078ThermalCompletion {
  readonly schema:
...
interface PreparedShape {
  readonly spec: Opt0078ShapeSpec;
  readonly activation: GPUBuffer;
  readonly weight: GPUBuffer;
  readonly output: GuardedOutput;
  readonly dispatches: Readonly<Record<Opt0078Arm, AceGemmDispatch>>;
...
interface PreparedHarness {
  ...
  readonly candidateKernel: AceOpt0078DenseWeightMulticastKernel;
...
const EXPERIMENT_ID = "OPT-0078" as const;
const RECEIPT_SCHEMA = "ace-opt-0078-dense-weight-multicast-primitive-v1";
...
const REQUIRED_SPEEDUP = 1.12;
const REQUIRED_SAVING_MILLISECONDS = 20.8334;
...
const TIMING_ROUNDS = Object.freeze(Array.from({ length: 8 }, (_, index) => {
  ...
  const armOrder: readonly Opt0078Arm[] = index % 2 === 0
    ? Object.freeze(["current", "candidate"])
    : Object.freeze(["candidate", "current"]);
  return Object.freeze({ roundIndex: index, shapeOrder, armOrder });
}));

export function buildOpt0078ShapeSpecs(): readonly Opt0078ShapeSpec[] {
  return SHAPE_SPECS;
}

export function buildOpt0078TimingRounds(): typeof TIMING_ROUNDS {
  return TIMING_ROUNDS;
}

export function summarizeOpt0078Timing(
  inputs: readonly Opt0078TimingInput[],
): Readonly<Record<string, unknown>> {
  if (inputs.length !== SHAPE_SPECS.length) {
    throw new Error("OPT-0078 timing requires all four exact M2250 shapes");
  }
  const strata = inputs.map((input, index) => {
    const spec = SHAPE_SPECS[index];
    if (spec === undefined || input.id !== spec.id) {
      throw new Error("OPT-0078 timing shape order changed");
    }
    ...
    const summarizeArm = (samples: readonly Opt0078TimestampSample[]) => {
  ...
  const weightedRounds = Array.from({ length: TIMING_ROUNDS.length },
    (_, roundIndex) => {
      const score = (arm: Opt0078Arm, metric: "gpuMilliseconds" |
        ...
        "feedForwardMultiplicity") => inputs.reduce((sum, input, index) => {
          ...
          if (sample === undefined || spec === undefined) {
            throw new Error("OPT-0078 weighted round sample missing");
          }
...
export function parseOpt0078ThermalLaunchGate(
  ...
  if (source !== THERMAL_SOURCE || command !== THERMAL_COMMAND ||
    ...
    throw new Error("OPT-0078 fresh continuous nominal thermal launch gate failed");
  }
...
export function parseOpt0078ThermalCompletion(
  parameters: URLSearchParams,
  launch: Opt0078ThermalLaunchGate,
  cleanupCompletedAtEpochMilliseconds: number,
): Opt0078ThermalCompletion {
  ...
  if (schema !== THERMAL_TRACE_SCHEMA || !/^[0-9a-f]{64}$/u.test(sha256) ||
    ...
    throw new Error("OPT-0078 complete through-cleanup thermal trace failed");
  }
...
function install(): void {
  ...
  runButton.addEventListener("click", () => {
    ...
    let thermalLaunch: Opt0078ThermalLaunchGate;
    try {
      thermalLaunch = parseOpt0078ThermalLaunchGate(
  ...
  finalizeButton.addEventListener("click", () => {
    ...
    const thermalLaunch = pending["thermalLaunch"] as
      Opt0078ThermalLaunchGate;
    try {
      const thermalCompletion = parseOpt0078ThermalCompletion(
        fieldParameters("#thermal-completion"),
        thermalLaunch,
        cleanupCompletedAtEpochMilliseconds,
      );
      const passed = pending["inPagePassed"] === true;
      const disposition = classifyOpt0078PrimitiveDisposition(pending);
      ...
      progress.textContent = passed
        ...
        : "completed — exact weight multicast stopped at its frozen gate";
      pending = undefined;
  ...
  downloadButton.addEventListener("click", () => {
    ...
    anchor.href = url;
    anchor.download = "opt-0078-dense-weight-multicast-receipt.json";
    anchor.click();
...
async function prepareHarness(
  ...
  const device = await adapter.requestDevice({
    label: "ace-opt-0078-dense-multicast-profile-device",
  ...
  const owners = (() => {
    ...
    let partialCandidate: AceOpt0078DenseWeightMulticastKernel | undefined;
    let partialQuerySet: GPUQuerySet | undefined;
    try {
      partialCurrent = AceOpt0009DenseGemmKernel.create(device, capability);
      partialCandidate = AceOpt0078DenseWeightMulticastKernel.create(
        device, capability);
      partialQuerySet = device.createQuerySet({
        label: "opt0078-dense-pass-timestamps", type: "timestamp", count: 2,
      });
      const queryResolve = tracker.create(device, {
        label: "opt0078-timestamp-resolve", size: TIMESTAMP_QUERY_BYTES,
        usage: GPUBufferUsage.QUERY_RESOLVE | GPUBufferUsage.COPY_SRC,
      });
      const queryReadback = tracker.create(device, {
        label: "opt0078-timestamp-readback", size: TIMESTAMP_QUERY_BYTES,
      ...
      if (firstShape !== undefined) {
        try {
          await candidateKernel.createDispatch(
            "opt0078-post-destroy-rejection",
  ...
  try {
    ...
    if (!correctness.passed) {
      throw new Error("OPT-0078 full-output raw-U32 correctness gate failed");
    }
    ...
    for (const [index, shape] of shapes.entries()) {
      const order: readonly Opt0078Arm[] = index % 2 === 0
    ...
    if (uncapturedErrors.length !== 0 || deviceLosses.length !== 0) {
      throw new Error("OPT-0078 GPU failure during warmup");
    }
...
async function prepareShape(
  ...
  candidateKernel: AceOpt0078DenseWeightMulticastKernel,
  spec: Opt0078ShapeSpec,
  ...
  const activation = tracker.create(device, {
    label: `opt0078-${spec.id}-activation`,
    size: activationBytes,
    usage: GPUBufferUsage.STORAGE,
    mappedAtCreation: true,
  });
  const weight = tracker.create(device, {
    label: `opt0078-${spec.id}-packed-weight`,
    size: weightBytes,
    usage: GPUBufferUsage.STORAGE,
    mappedAtCreation: true,
  });
  ...
  try {
    ...
    const current = await currentKernel.createDispatch(
      `opt0078-${spec.id}-current-opt0009`, spec.shape, bindings);
    const candidate = await candidateKernel.createDispatch(
      `opt0078-${spec.id}-candidate-multicast`, spec.shape, bindings);
    if (current.weightLayout !== ACE_DIT_DENSE_FP16_TILE_LAYOUT ||
      candidate.weightLayout !== ACE_DIT_DENSE_FP16_TILE_LAYOUT) {
      throw new Error(`OPT-0078 ${spec.id} changed native packed weight layout`);
    }
...
async function verifyShape(
  ...
  dispatches: Readonly<Record<Opt0078Arm, AceGemmDispatch>>,
...
async function executeCorrectness(
  ...
  if (output.readback.mapState !== "unmapped") {
    throw new Error("OPT-0078 output readback must be unmapped");
  }
...
function compareSnapshots(
  ...
  if (expected.words.length !== actual.words.length) {
    throw new Error(`OPT-0078 ${spec.id} output length changed`);
  }
...
function createGuardedOutput(
  ...
  const prefill = tracker.create(device, {
    label: `opt0078-${spec.id}-prefill`, size: totalBytes,
  ...
  const buffer = tracker.create(device, {
    label: `opt0078-${spec.id}-guarded-output`, size: totalBytes,
    usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST |
      GPUBufferUsage.COPY_SRC,
  });
  const readback = tracker.create(device, {
    label: `opt0078-${spec.id}-readback`, size: totalBytes,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
  });
...
function fillActivation(values: Float32Array, spec: Opt0078ShapeSpec): void {
  let physical = 0;
...
function fillPackedWeight(values: Uint16Array, spec: Opt0078ShapeSpec): void {
  let physical = 0;
  ...
  if (physical !== values.length) {
    throw new Error(`OPT-0078 ${spec.id} packed weight fill changed`);
  }
}
...
async function runTiming(
  ...
  thermalLaunch: Opt0078ThermalLaunchGate,
  update: (message: string) => void,
): Promise<Readonly<Record<string, unknown>>> {
  const samples = new Map<Opt0078ShapeSpec["id"],
    Record<Opt0078Arm, Opt0078TimestampSample[]>>(
  ...
  for (const round of TIMING_ROUNDS) {
    for (const [shapePosition, shapeIndex] of round.shapeOrder.entries()) {
      ...
      if (shape === undefined) {
        throw new Error("OPT-0078 timing shape rotation changed");
      }
  ...
  if (prepared.uncapturedErrors.length !== 0 ||
    ...
    throw new Error("OPT-0078 observed a timing GPU error or device loss");
  }
  ...
  const timing = summarizeOpt0078Timing(timingInputs);
...
async function timeDispatch(
  ...
  spec: Opt0078ShapeSpec,
  arm: Opt0078Arm,
): Promise<Opt0078TimestampSample> {
  if (queryReadback.mapState !== "unmapped") {
    throw new Error("OPT-0078 timestamp readback must be unmapped");
  }
  const encoder = device.createCommandEncoder({
    label: `opt0078-${spec.id}-${arm}-sample`,
  });
  const pass = encoder.beginComputePass({
    label: `opt0078-${spec.id}-${arm}-timestamped-compute`,
  ...
  if (timestampEnd <= timestampBegin) {
    throw new Error(`OPT-0078 ${spec.id} ${arm} non-positive GPU timestamp`);
  }
  ...
  if (!Number.isSafeInteger(gpuElapsedNanoseconds) || gpuMilliseconds <= 0 ||
    ...
    throw new Error(`OPT-0078 ${spec.id} ${arm} invalid timing sample`);
  }
...
async function buildIdentity(
  ...
  const generatedShaders = await Promise.all(SHAPE_SPECS.map(async (spec) => {
    ...
    const candidatePlan = planAceOpt0078DenseWeightMulticast(spec.shape);
    return Object.freeze({ id: spec.id,
      ...
      candidateWgslSha256: await sha256Text(
        aceOpt0078DenseWeightMulticastWgsl(spec.shape)),
      ...
      identicalWorkgroups:
        currentPlan.workgroupCount === candidatePlan.workgroupCount,
      identicalOutputRanges:
        JSON.stringify(currentPlan.outputRanges) ===
          JSON.stringify(candidatePlan.outputRanges),
      identicalScheduledMultiplyAdds:
        currentPlan.outputRanges[0]?.multiplyAdds ===
          candidatePlan.outputRanges[0]?.multiplyAdds,
    });
  }));
  return Object.freeze({ registrationCommit: REGISTRATION_COMMIT,
    ...
    fixtureVersion: "opt0078-full-m2250-five-edge-families-v1",
...
function planReceipt(plan: ReturnType<typeof planAceOpt0009DenseGemm> |
  ReturnType<typeof planAceOpt0078DenseWeightMulticast>):
Readonly<Record<string, unknown>> {
...
export function classifyOpt0078PrimitiveDisposition(
  receipt: Readonly<Record<string, unknown>>,
): string {
  if (receipt["inPagePassed"] === true) {
    return "positive-primitive-diagnostic-profile-authorized";
  }
  ...
  const gates = timing?.["gates"] as
    Readonly<Record<string, unknown>> | undefined;
  const shapeGateSets = strata?.map((stratum) => Object.values(
    stratum["gates"] as Readonly<Record<string, unknown>>,
  ).map(Boolean)) ?? [];
  ...
  if (anyConsistentlyRegressingShape || clearSubTwoSecondProjection) {
    return "negative-stop-exact-weight-multicast-mechanism";
  }
  return "inconclusive-directional-or-wall-gpu-evidence";
}
...
class BufferTracker {
  ...
  unmap(buffer: GPUBuffer): void {
    if (buffer.mapState !== "mapped") {
      throw new Error("OPT-0078 attempted to unmap an unmapped buffer");
    }
    buffer.unmap();
    this.unmaps += 1;
    this.activeMaps -= 1;
    if (this.activeMaps < 0) {
      throw new Error("OPT-0078 map accounting became negative");
    }
  }
...
function requireTimestampSamples(
  samples: readonly Opt0078TimestampSample[],
  label: string,
): void {
  if (samples.length !== TIMING_ROUNDS.length || samples.some((sample) =>
    ...
    throw new Error(`OPT-0078 ${label} requires eight valid timestamp samples`);
  }
}
...
function mean(values: readonly number[]): number {
  if (values.length !== TIMING_ROUNDS.length || values.some((value) =>
    ...
    throw new Error("OPT-0078 mean requires eight finite positive samples");
  }
...
function median(values: readonly number[]): number {
  if (values.length !== TIMING_ROUNDS.length || values.some((value) =>
    !Number.isFinite(value) || value <= 0)) {
    throw new Error("OPT-0078 median requires eight finite positive samples");
  }
...
function requiredParameter(parameters: URLSearchParams, name: string): string {
  ...
  if (value === null || value.trim() === "") {
    throw new Error(`OPT-0078 field ${name} is missing`);
  }
  return value.trim();
}
...
function requiredFiniteParameter(
  ...
  if (!Number.isFinite(value)) {
    throw new Error(`OPT-0078 field ${name} is invalid`);
  }
  return value;
}
...
function requiredIntegerParameter(
  ...
  if (!Number.isSafeInteger(value)) {
    throw new Error(`OPT-0078 field ${name} must be an integer`);
  }
  return value;
}
...
function requireAdapter(adapter: GPUAdapter): void {
  ...
  if (!adapter.features.has("shader-f16") ||
    ...
    throw new Error(
      "OPT-0078 requires timestamp-query, shader-f16, fixed32 subgroups, WG256, 16KiB workgroup storage, and full-shape storage limits",
    );
  }
}
...
function tflops(multiplyAdds: number, milliseconds: number): number {
  if (!Number.isSafeInteger(multiplyAdds) || multiplyAdds <= 0 ||
    ...
    throw new Error("OPT-0078 TFLOP/s inputs must be finite and positive");
  }
...
function requireLittleEndianHost(): void {
  ...
  if (new Uint8Array(words.buffer)[0] !== 0x02) {
    throw new Error("OPT-0078 fixtures require a little-endian host");
  }
}
...
function publish(
  ...
  window.__ACE_OPT0078_RESULT__ = receipt;
