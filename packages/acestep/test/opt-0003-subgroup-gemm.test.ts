...
import {
  ...
  aceOpt0003SubgroupGemmWgsl,
  packAceOpt0003SubgroupBf16Weight,
  planAceOpt0003SubgroupGemm,
  unpackAceOpt0003SubgroupBf16Weight,
...
describe("OPT-0003 packed-BF16 subgroup GEMM candidate", () => {
  it("plans the exact fixed geometry and bounded principal-shape ranges", () => {
    const plan = planAceOpt0003SubgroupGemm({
      rows: 2_250,
      inner: 2_048,
      columns: 6_144,
    });
    ...
    expect(ACE_OPT_0003_SUBGROUP_GEMM_WORKGROUP_BYTES).toBe(0);
  });
  ...
  it.each([
    ...
    expect(() => planAceOpt0003SubgroupGemm(shape)).toThrow(shape.reason);
  });

  it("prepacks and unpacks every BF16 bit in the declared physical order", () => {
    const shape = { rows: 3, inner: 64, columns: 256 };
    const scalarCount = shape.inner * shape.columns;
    const logical = new Uint32Array(scalarCount / 2);
    for (let scalar = 0; scalar < scalarCount; scalar += 1) {
      setPackedBits(logical, scalar, fixtureBits(scalar));
    }
    const specialBits = [
      0x0000, // +0
      0x8000, // -0
      0x7f80, // +infinity
      0xff80, // -infinity
      0x7fc1, // positive quiet NaN payload
      0xffc1, // negative quiet NaN payload
      0x7f81, // positive signaling NaN payload
      0xff81, // negative signaling NaN payload
    ] as const;
    for (let index = 0; index < specialBits.length; index += 1) {
      setPackedBits(logical, index * 2_049, specialBits[index]!);
    }
    const before = logical.slice();
    const physical = packAceOpt0003SubgroupBf16Weight(logical, shape);
    expect(logical).toEqual(before);
    expect(physical).toHaveLength(logical.length);

    for (const [column, inner] of [
      [0, 0],
      [1, 0],
      [127, 31],
      [0, 32],
      ...
      const physicalScalar = physicalScalarIndex(shape, column, inner);
      expect(readPackedBits(physical, physicalScalar)).toBe(
        readPackedBits(logical, column * shape.inner + inner),
      );
    }
    expect(unpackAceOpt0003SubgroupBf16Weight(physical, shape)).toEqual(logical);
    const roundTrip = unpackAceOpt0003SubgroupBf16Weight(physical, shape);
    for (let index = 0; index < specialBits.length; index += 1) {
      expect(readPackedBits(roundTrip, index * 2_049)).toBe(specialBits[index]);
    }
  });

  it("round-trips all 65,536 BF16 bit patterns without numerical conversion", () => {
    const shape = { rows: 1, inner: 512, columns: 128 };
    const logical = new Uint32Array(65_536 / 2);
    for (let bits = 0; bits <= 0xffff; bits += 1) {
      setPackedBits(logical, bits, bits);
    }
    const physical = packAceOpt0003SubgroupBf16Weight(logical, shape);
    const roundTrip = unpackAceOpt0003SubgroupBf16Weight(physical, shape);
    expect(roundTrip).toEqual(logical);
    for (let bits = 0; bits <= 0xffff; bits += 1) {
      expect(readPackedBits(roundTrip, bits)).toBe(bits);
    }
  });

  it("preflights exact packed lengths before allocating or transforming", () => {
    const shape = { rows: 1, inner: 32, columns: 128 };
    expect(() =>
      packAceOpt0003SubgroupBf16Weight(new Uint32Array(2_047), shape)
    ).toThrow(/exactly 2048 packed BF16 words/);
    expect(() =>
      unpackAceOpt0003SubgroupBf16Weight(new Uint32Array(2_049), shape)
    ).toThrow(/exactly 2048 packed BF16 words/);
    expect(() =>
      packAceOpt0003SubgroupBf16Weight(
        new Float32Array(2_048) as unknown as Uint32Array,
        shape,
      )
    ).toThrow(/exactly 2048 packed BF16 words/);
  });
  ...
  it("emits direct packed BF16, fixed-32 broadcasts, and exact K-order adds", () => {
    const source = aceOpt0003SubgroupGemmWgsl(
  ...
  it("fails closed without a reported fixed-32 subgroup capability", () => {
    ...
    expect(() =>
      AceOpt0003SubgroupGemmKernel.create(noFeature, {
        subgroupMinSize: 32,
        subgroupMaxSize: 32,
      })
    ...
    expect(() =>
      AceOpt0003SubgroupGemmKernel.create(variable, {
        subgroupMinSize: 4,
        subgroupMaxSize: 64,
      })
    ).toThrow(/fixed 32-lane subgroups/);
    ...
    expect(() =>
      AceOpt0003SubgroupGemmKernel.create(tooSmall, {
        subgroupMinSize: 32,
        subgroupMaxSize: 32,
      })
  ...
  it("compiles, binds, and encodes every bounded range", async () => {
    ...
    const kernel = AceOpt0003SubgroupGemmKernel.create(device, {
      subgroupMinSize: 32,
      subgroupMaxSize: 32,
    });
...
expect(ACE_OPT_0003_SUBGROUP_SIZE).toBe(32);
expect(ACE_OPT_0003_SUBGROUP_GEMM_TILE_ROWS).toBe(32);
expect(ACE_OPT_0003_SUBGROUP_GEMM_TILE_COLUMNS).toBe(128);
expect(ACE_OPT_0003_SUBGROUP_GEMM_TILE_INNER).toBe(32);
expect(ACE_OPT_0003_SUBGROUP_GEMM_WORKGROUP_SIZE).toBe(128);

vi.stubGlobal("GPUBufferUsage", { UNIFORM: 1 << 6 });

function fixtureBits(scalar: number): number {
  return (scalar * 40_503 + 0x7fc1) & 0xffff;
}
...
function readPackedBits(values: Uint32Array, scalar: number): number {
  const pair = values[scalar >> 1]!;
  return (scalar & 1) === 0 ? pair & 0xffff : pair >>> 16;
}

function setPackedBits(values: Uint32Array, scalar: number, bits: number): void {
  const word = scalar >> 1;
  values[word] = (scalar & 1) === 0
    ? ((values[word]! & 0xffff0000) | (bits & 0xffff)) >>> 0
    : ((values[word]! & 0xffff) | ((bits & 0xffff) << 16)) >>> 0;
}

function physicalScalarIndex(
  shape: Readonly<{ inner: number; columns: number }>,
  column: number,
  inner: number,
): number {
  const columnTile = Math.floor(column / 128);
  const columnInTile = column % 128;
  const innerTile = Math.floor(inner / 32);
  const innerInTile = inner % 32;
  const innerTiles = shape.inner / 32;
  return (
    ((columnTile * innerTiles + innerTile) * 32 + innerInTile) * 128 +
    columnInTile
  );
}
