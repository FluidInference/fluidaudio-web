...
export const ACE_OPT_0019_DENSE_COOPERATIVE_KERNEL_SET_ID =
  "opt-0019-m64-n128-k16-cooperative-fp16-fp32-v1" as const;
export const ACE_OPT_0019_DENSE_TILE_ROWS = 64;
export const ACE_OPT_0019_DENSE_TILE_COLUMNS = 128;
export const ACE_OPT_0019_DENSE_TILE_INNER = 16;
export const ACE_OPT_0019_DENSE_WORKGROUP_SIZE_X = 16;
export const ACE_OPT_0019_DENSE_WORKGROUP_SIZE_Y = 16;
export const ACE_OPT_0019_DENSE_WORKGROUP_SIZE = 256;
export const ACE_OPT_0019_DENSE_ROWS_PER_THREAD = 4;
export const ACE_OPT_0019_DENSE_COLUMNS_PER_THREAD = 8;
export const ACE_OPT_0019_DENSE_ACCUMULATORS_PER_THREAD = 32;
export const ACE_OPT_0019_DENSE_INPUT_PANEL_STRIDE = 17;
export const ACE_OPT_0019_DENSE_WEIGHT_PANEL_STRIDE = 132;
export const ACE_OPT_0019_DENSE_BARRIERS_PER_INNER_TILE = 2;
...
const INPUT_PANEL_ELEMENTS =
  ACE_OPT_0019_DENSE_TILE_ROWS * ACE_OPT_0019_DENSE_INPUT_PANEL_STRIDE;
const WEIGHT_PANEL_ELEMENTS =
  ACE_OPT_0019_DENSE_TILE_INNER * ACE_OPT_0019_DENSE_WEIGHT_PANEL_STRIDE;
export const ACE_OPT_0019_DENSE_WORKGROUP_STORAGE_BYTES =
...
export interface AceOpt0019DenseCooperativePlan
  extends AceCooperativeGemmPlan {
  readonly kernelSetId:
    typeof ACE_OPT_0019_DENSE_COOPERATIVE_KERNEL_SET_ID;
  readonly tileRows: typeof ACE_OPT_0019_DENSE_TILE_ROWS;
  readonly tileColumns: typeof ACE_OPT_0019_DENSE_TILE_COLUMNS;
  readonly tileInner: typeof ACE_OPT_0019_DENSE_TILE_INNER;
  readonly workgroupSize: typeof ACE_OPT_0019_DENSE_WORKGROUP_SIZE;
  readonly workgroupSizeX: typeof ACE_OPT_0019_DENSE_WORKGROUP_SIZE_X;
  readonly workgroupSizeY: typeof ACE_OPT_0019_DENSE_WORKGROUP_SIZE_Y;
  readonly rowsPerThread: typeof ACE_OPT_0019_DENSE_ROWS_PER_THREAD;
  readonly columnsPerThread: typeof ACE_OPT_0019_DENSE_COLUMNS_PER_THREAD;
  readonly accumulatorsPerThread:
    typeof ACE_OPT_0019_DENSE_ACCUMULATORS_PER_THREAD;
...
export interface AceOpt0019DenseCooperativeDispatch extends AceGemmDispatch {
  readonly kernelSetId:
    typeof ACE_OPT_0019_DENSE_COOPERATIVE_KERNEL_SET_ID;
  readonly weightLayout: typeof ACE_DIT_DENSE_FP16_TILE_LAYOUT;
  readonly plan: AceOpt0019DenseCooperativePlan;
}
...
/**
 * Isolated OPT-0019 benchmark owner. It consumes the existing N256/K32 FP16
 * payload, stages exact N128/K16 quadrants, and never participates in a
 * production selector or fallback.
 */
export class AceOpt0019DenseCooperativePanelsKernel implements AceGemmKernel {
  ...
  static create(device: GPUDevice): AceOpt0019DenseCooperativePanelsKernel {
    requireKernelDevice(device);
    return new AceOpt0019DenseCooperativePanelsKernel(device);
  }
  ...
  async createDispatch(
  ...
  ): Promise<AceOpt0019DenseCooperativeDispatch> {
    this.requireLive();
    if (bindings.bias !== undefined) {
      throw new RangeError(
        "OPT-0019 repeated-layer cooperative dense GEMMs reject bias",
      );
    }
    ...
    const bindGroup = this.bindGroupFor(
      shapeKey(plan),
      `${label}-opt-0019-cooperative-bindings`,
    ...
    return Object.freeze({
      ...
      encodeRange(pass: GPUComputePassEncoder, rangeIndex: number): void {
        if (rangeIndex !== 0) {
          throw new RangeError(`${label} OPT-0019 dense range must be zero`);
        }
        encode(pass);
      },
      encode,
  ...
  private pipelineFor(
    plan: AceOpt0019DenseCooperativePlan,
  ): Promise<CompiledKernel> {
    const key = shapeKey(plan);
    const existing = this.pipelines.get(key);
    if (existing !== undefined) return existing;
    const created = compileCooperativePanels(this.device, plan);
  ...
  private requireLive(): void {
    if (this.destroyed) {
      throw new Error("OPT-0019 cooperative dense kernel was destroyed");
    }
  }
}

export function planAceOpt0019DenseCooperativePanels(
  shape: AceGemmShape,
): AceOpt0019DenseCooperativePlan {
  ...
  if (rows !== 2_250) {
    throw new RangeError("OPT-0019 cooperative dense requires exact M2250");
  }
  if (!PRODUCTION_DENSE_SHAPES.has(`${inner}x${columns}`)) {
    throw new RangeError(
      `OPT-0019 cooperative dense rejects non-production K${inner}/N${columns}`,
    );
  }
  if (
    inner % ACE_OPT_0019_DENSE_TILE_INNER !== 0 ||
    inner % PACKED_TILE_INNER !== 0 ||
    columns % ACE_OPT_0019_DENSE_TILE_COLUMNS !== 0 ||
    columns % PACKED_TILE_COLUMNS !== 0
  ) {
    throw new RangeError(
      "OPT-0019 cooperative dense requires exact K16/N128 panel multiples and N256/K32 packed tiles",
    );
  }
  ...
  const rowTiles = Math.ceil(rows / ACE_OPT_0019_DENSE_TILE_ROWS);
  const columnTiles = columns / ACE_OPT_0019_DENSE_TILE_COLUMNS;
  const innerTiles = inner / ACE_OPT_0019_DENSE_TILE_INNER;
  ...
  return Object.freeze({
    ...
    tileRows: ACE_OPT_0019_DENSE_TILE_ROWS,
    tileColumns: ACE_OPT_0019_DENSE_TILE_COLUMNS,
    tileInner: ACE_OPT_0019_DENSE_TILE_INNER,
    workgroupSize: ACE_OPT_0019_DENSE_WORKGROUP_SIZE,
    workgroupSizeX: ACE_OPT_0019_DENSE_WORKGROUP_SIZE_X,
    workgroupSizeY: ACE_OPT_0019_DENSE_WORKGROUP_SIZE_Y,
    ...
    outputRangeCount: 1,
    outputRanges,
    inputPanelStride: ACE_OPT_0019_DENSE_INPUT_PANEL_STRIDE,
    inputPanelElements: INPUT_PANEL_ELEMENTS,
    weightPanelStride: ACE_OPT_0019_DENSE_WEIGHT_PANEL_STRIDE,
    weightPanelElements: WEIGHT_PANEL_ELEMENTS,
    workgroupStorageBytes: ACE_OPT_0019_DENSE_WORKGROUP_STORAGE_BYTES,
    barriersPerWorkgroup,
    barrierEvents,
    estimatedGlobalActivationBytes,
    estimatedGlobalWeightBytes,
...
export function aceOpt0019DenseCooperativePanelsWgsl(
  shape: AceGemmShape,
): string {
  const plan = planAceOpt0019DenseCooperativePanels(shape);
  const accumulators = Array.from(
    { length: ACE_OPT_0019_DENSE_ROWS_PER_THREAD },
    (_, row) =>
  ...
  const contractions = Array.from(
    { length: ACE_OPT_0019_DENSE_ROWS_PER_THREAD },
    (_, row) => /* wgsl */ `
      let a${row} = f32(input_panel[
        (owned_panel_row + ${row}u) * INPUT_PANEL_STRIDE + inner_in_tile
      ]);
      acc${row}_0 = acc${row}_0 + vec4<f32>(a${row}) * b0;
      acc${row}_1 = acc${row}_1 + vec4<f32>(a${row}) * b1;`,
  ).join("");
...
const TILE_ROWS: u32 = ${ACE_OPT_0019_DENSE_TILE_ROWS}u;
const TILE_COLUMNS: u32 = ${ACE_OPT_0019_DENSE_TILE_COLUMNS}u;
const TILE_INNER: u32 = ${ACE_OPT_0019_DENSE_TILE_INNER}u;
const INPUT_PANEL_STRIDE: u32 = ${ACE_OPT_0019_DENSE_INPUT_PANEL_STRIDE}u;
const WEIGHT_PANEL_STRIDE: u32 = ${ACE_OPT_0019_DENSE_WEIGHT_PANEL_STRIDE}u;
...
${accumulators}
  ...
  for (var inner_tile = 0u; inner_tile < ${plan.innerTiles}u; inner_tile += 1u) {
    ...
    let weight_panel_base =
      local_id.y * WEIGHT_PANEL_STRIDE + local_id.x * 8u;
    weight_panel[weight_panel_base] = packed_low.x;
    weight_panel[weight_panel_base + 1u] = packed_low.y;
    weight_panel[weight_panel_base + 2u] = packed_low.z;
    weight_panel[weight_panel_base + 3u] = packed_low.w;
    weight_panel[weight_panel_base + 4u] = packed_high.x;
    weight_panel[weight_panel_base + 5u] = packed_high.y;
    weight_panel[weight_panel_base + 6u] = packed_high.z;
    weight_panel[weight_panel_base + 7u] = packed_high.w;

    workgroupBarrier();
    for (var inner_in_tile = 0u; inner_in_tile < TILE_INNER; inner_in_tile += 1u) {
      let weight_base =
        inner_in_tile * WEIGHT_PANEL_STRIDE + owned_panel_column;
      let b0 = vec4<f32>(
        f32(weight_panel[weight_base]),
        f32(weight_panel[weight_base + 1u]),
        f32(weight_panel[weight_base + 2u]),
        f32(weight_panel[weight_base + 3u])
      );
      let b1 = vec4<f32>(
        f32(weight_panel[weight_base + 4u]),
        f32(weight_panel[weight_base + 5u]),
        f32(weight_panel[weight_base + 6u]),
        f32(weight_panel[weight_base + 7u])
      );
${contractions}
    }
    workgroupBarrier();
...
async function compileCooperativePanels(
  device: GPUDevice,
  plan: AceOpt0019DenseCooperativePlan,
): Promise<CompiledKernel> {
  const label = `ace-opt-0019-dense-${plan.rows}x${plan.inner}x${plan.columns}`;
  const module = await checkedShaderModule(
    device,
    label,
    aceOpt0019DenseCooperativePanelsWgsl(plan),
  );
  const bindingBytes = [
...
async function checkedShaderModule(
  ...
  if (errors.length > 0) {
    throw new Error(
      `OPT-0019 cooperative dense WGSL failed: ${errors.map((message) =>
...
function requireKernelDevice(device: GPUDevice): void {
  if (!device.features.has("shader-f16")) {
    throw new Error("OPT-0019 cooperative dense requires WebGPU shader-f16");
  }
  ...
  if (
    ...
    invocations < ACE_OPT_0019_DENSE_WORKGROUP_SIZE ||
    sizeX < ACE_OPT_0019_DENSE_WORKGROUP_SIZE_X ||
    sizeY < ACE_OPT_0019_DENSE_WORKGROUP_SIZE_Y
  ) {
    throw new Error(
      "OPT-0019 cooperative dense requires WG256 with a 16x16 workgroup",
    );
  }
  if (
    !Number.isSafeInteger(storage) ||
    storage < ACE_OPT_0019_DENSE_WORKGROUP_STORAGE_BYTES
  ) {
    throw new Error(
      `OPT-0019 cooperative dense requires ${ACE_OPT_0019_DENSE_WORKGROUP_STORAGE_BYTES} workgroup-storage bytes`,
    );
  }
}
...
function requireDispatchDimensions(
  ...
  if (
    ...
    throw new RangeError(
      "OPT-0019 cooperative dense exceeds the device dispatch dimension",
    );
  }
}
...
function requireBufferLimits(
  ...
  if (
    ...
    throw new RangeError(
      "OPT-0019 cooperative dense device reported invalid buffer limits",
    );
  }
  for (const [name, bytes] of resources) {
    if (bytes > maximumBinding || bytes > maximumBuffer) {
      throw new RangeError(
        `OPT-0019 cooperative dense ${name} exceeds the device buffer limits`,
      );
    }
  }
}
...
function requireStorageBinding(
  ...
  if (!isValidGpuAlignment(alignment)) {
    throw new Error(
      "OPT-0019 cooperative dense device reported an invalid storage alignment",
    );
  }
...
function requirePositiveSafeInteger(value: number, label: string): void {
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new RangeError(
      `OPT-0019 cooperative dense ${label} must be a positive safe integer`,
    );
  }
}
...
function checkedProduct(left: number, right: number, label: string): number {
  ...
  if (!Number.isSafeInteger(product)) {
    throw new RangeError(
      `OPT-0019 cooperative dense ${label} is not a safe integer`,
    );
  }
  return product;
}
...
function requireWgslIndexable(elements: number, label: string): void {
  if (elements > MAX_WGSL_U32) {
    throw new RangeError(
      `OPT-0019 cooperative dense ${label} exceeds WGSL's u32 indexing domain`,
    );
  }
}
