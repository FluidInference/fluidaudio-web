...
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_KERNEL_ID =
  "ace-vae-fp16-opt-0024-direct-native-oki-fp16-dot4-k7-conv1d-v1" as const;
export const
...
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_SIZE = 32;
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_WORKGROUP_SIZE = 128;
export const
  ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUPS_PER_WORKGROUP = 4;
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_ROWS = 8;
export const
  ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_OUTPUTS_PER_LANE = 4;
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_TILE_ROWS = 32;
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_TILE_CHANNELS = 128;
export const ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_CIN_VECTOR_WIDTH = 4;
...
export interface AceOpt0024VaeConv1dDirectDot4SubgroupDispatch {
...
interface CompiledAceOpt0024VaeConv1dDirectDot4Subgroup {
...
/**
 * Isolated OPT-0024 benchmark owner for biased FP16-output K7 Conv1D.
 *
 * This keeps the shipped fixed32 8-row by 128-channel subgroup ownership and
 * native NLC / O-K-I payloads. It changes only the reduction primitive: each
 * increasing Cin4 group produces FP16 dot4 partials that are widened once and
 * added to eight FP32 vec4 accumulators. There is no fallback or production
 * route in this owner.
 */
export class AceOpt0024VaeConv1dDirectDot4SubgroupKernel {
  private readonly pipelines = new Map<
    string,
    Promise<CompiledAceOpt0024VaeConv1dDirectDot4Subgroup>
  >();
  ...
  static create(
    ...
    return new AceOpt0024VaeConv1dDirectDot4SubgroupKernel(device);
  }
  ...
  async createDispatch(
  ...
  ): Promise<AceOpt0024VaeConv1dDirectDot4SubgroupDispatch> {
    this.requireLive();
    const plan = planAceFp16VaeConv1d(shape, outputStorage);
    requireCandidateBoundary(plan, bindings.bias !== undefined);
    const outputRange = planAceOpt0024VaeConv1dDirectDot4SubgroupRange(
      plan,
      range,
    );
    ...
    if (bindGroup === undefined) {
      bindGroup = this.device.createBindGroup({
        label: `${label}-opt-0024-direct-dot4-subgroup-bindings`,
  ...
  private pipelineFor(
    ...
    const created = compileAceOpt0024VaeConv1dDirectDot4Subgroup(
      this.device,
      plan,
    );
  ...
  private requireBindings(
    ...
    if (bias === undefined) {
      throw new RangeError("OPT-0024 direct-dot4 Conv1D requires bias");
    }
  ...
  private requireDeviceLimits(
    ...
    if (
      ...
      throw new RangeError(
        "OPT-0024 direct-dot4 Conv1D range exceeds the device dispatch dimension",
      );
    }
    ...
    if (
      ...
      throw new RangeError(
        "OPT-0024 direct-dot4 Conv1D device reported invalid buffer limits",
      );
    }
    for (const [name, bytes] of [
      ...
      if (bytes > maximumBinding) {
        throw new RangeError(
          `OPT-0024 direct-dot4 Conv1D ${name} exceeds the device storage binding limit`,
        );
      }
      if (bytes > maximumBuffer) {
        throw new RangeError(
          `OPT-0024 direct-dot4 Conv1D ${name} exceeds the device buffer limit`,
        );
      }
    }
  }
  ...
  private requireLive(): void {
    if (this.destroyed) {
      throw new Error("OPT-0024 direct-dot4 Conv1D kernel was destroyed");
    }
  }
}

export function planAceOpt0024VaeConv1dDirectDot4SubgroupRange(
  plan: AceFp16VaeConv1dPlan,
...
export function aceOpt0024VaeConv1dDirectDot4SubgroupWgsl(
  shape: AceVaeConv1dShape,
  hasBias: boolean,
  outputStorage: AceFp16VaeConv1dOutputStorage,
): string {
...
async function compileAceOpt0024VaeConv1dDirectDot4Subgroup(
  device: GPUDevice,
  plan: AceFp16VaeConv1dPlan,
): Promise<CompiledAceOpt0024VaeConv1dDirectDot4Subgroup> {
  requireCandidateBoundary(plan, true);
  const label = `ace-opt-0024-vae-direct-dot4-subgroup-${convKey(plan)}`;
  const module = device.createShaderModule({
    label,
    code: aceOpt0024VaeConv1dDirectDot4SubgroupWgsl(
      plan,
      true,
      "float16",
    ),
  ...
  if (errors.length > 0) {
    throw new Error(
      `OPT-0024 direct-dot4 Conv1D WGSL failed: ${errors.map((message) =>
...
function requireFixedSubgroupDevice(
  ...
  if (!device.features.has("shader-f16")) {
    throw new Error("OPT-0024 direct-dot4 Conv1D requires WebGPU shader-f16");
  }
  if (
    ...
    throw new Error(
      "OPT-0024 direct-dot4 Conv1D requires reported fixed 32-lane subgroups",
    );
  }
  ...
  if (
    ...
    throw new Error(
      `OPT-0024 direct-dot4 Conv1D requires WG${ACE_OPT_0024_VAE_CONV1D_DIRECT_DOT4_SUBGROUP_WORKGROUP_SIZE}`,
    );
  }
}
...
function requireCandidateBoundary(
  ...
  if (plan.family !== "k7") {
    throw new RangeError("OPT-0024 direct-dot4 Conv1D supports only K7");
  }
  if (!hasBias) {
    throw new RangeError("OPT-0024 direct-dot4 Conv1D requires bias");
  }
  if (plan.outputStorage !== "float16") {
    throw new RangeError(
      "OPT-0024 direct-dot4 Conv1D requires FP16 internal output",
    );
  }
  if (
    plan.inputChannels %
    ...
    throw new RangeError(
      "OPT-0024 direct-dot4 Conv1D requires Cin divisible by 4",
    );
  }
  if (
    plan.outputChannels %
    ...
    throw new RangeError(
      "OPT-0024 direct-dot4 Conv1D requires Cout divisible by 128",
    );
  }
}
...
function requireStorageBinding(
  ...
  if (!isValidGpuAlignment(deviceAlignment)) {
    throw new Error(
      "OPT-0024 direct-dot4 Conv1D device reported an invalid storage alignment",
    );
  }
...
function requireRangeControlBinding(
  ...
  if (!isValidGpuAlignment(alignment)) {
    throw new Error(
      "OPT-0024 direct-dot4 Conv1D device reported an invalid uniform alignment",
    );
  }
  const bufferBytes = Number(binding.buffer.size);
  const offset = binding.offset ?? 0;
...
function convKey(plan: AceFp16VaeConv1dPlan): string {
  return [
    "opt-0024-direct-dot4-fixed32-subgroup-native-oki",
    plan.batch,
    plan.inputFrames,
    plan.inputChannels,
    plan.outputChannels,
