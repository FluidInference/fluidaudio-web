...
function flacToF32(path) {
  const buf = execFileSync("ffmpeg", ["-v", "quiet", "-i", path, "-ar", "16000", "-ac", "1", "-f", "s16le", "-"], { maxBuffer: 1 << 28 });
...
for (const u of sample) {
  ...
  totMs += Date.now() - t0;
  totDur += audio.length / 16000;
  const { err, n } = wer(u.ref, r.text);
  totErr += err;
  totN += n;
  done++;
