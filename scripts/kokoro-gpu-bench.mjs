...
const man = JSON.parse(readFileSync(`${A}/manifest.json`)); const cpu = {};
for (const k of Object.keys(man)) cpu[k] = rd(`${A}/${k}.bin`);
const sc = 1 / Math.sqrt(ALBERT_DIMS.HEAD_DIM);
for (let i = 0; i < cpu.q_w.length; i++) cpu.q_w[i] *= sc; for (let i = 0; i < cpu.q_b.length; i++) cpu.q_b[i] *= sc;
...
const beW = ctx.upload(rd(`${F}/be_w.bin`), fref.be_in, fref.be_out), beB = ctx.upload(rd(`${F}/be_b.bin`), 1, fref.be_out);
let ids;
if (process.env.LONG) {
  ...
  const a = [0]; for (const ch of phon) { const id = vocab[ch]; if (id !== undefined) a.push(id); } a.push(0);
  ids = Int32Array.from(a);
} else {
  const idb = Uint8Array.from(readFileSync(`${K0}/hello_ids.bin`)); ids = new Int32Array(idb.buffer, idb.byteOffset, idb.byteLength / 4);
}
...
if (process.env.LONG) {
  let nan = false, pk = 0, s2 = 0;
  for (let i = 0; i < wav.length; i++) { if (Number.isNaN(wav[i])) nan = true; pk = Math.max(pk, Math.abs(wav[i])); s2 += wav[i] ** 2; }
  console.log(`WebGPU LONG synth ${ms.toFixed(0)}ms for ${(wav.length / 24000).toFixed(2)}s audio → RTFx ${(wav.length / 24000 / (ms / 1000)).toFixed(1)}  NaN ${nan}  peak ${pk.toFixed(3)}  rms ${Math.sqrt(s2 / wav.length).toFixed(4)}`);
} else {
  ...
  console.log(`WebGPU synth ${ms.toFixed(0)}ms for ${(wav.length / 24000).toFixed(2)}s audio → RTFx ${(wav.length / 24000 / (ms / 1000)).toFixed(1)}  corr ${(dot / Math.sqrt(a * b)).toFixed(4)}`);
}
process.exit(0);
