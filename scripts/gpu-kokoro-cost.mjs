...
const Cin = 128, L = 9841, Cout = 128, K = 11, pad = 5; // dominant Kokoro resblock conv
...
async function bench(fn, label) {
  ...
  console.log(`${label.padEnd(14)} ${ms.toFixed(2)} ms  ${gfs.toFixed(0)} GFLOP/s  ->  ${CONV_GFLOP_TOTAL} GFLOP conv ≈ ${convMs.toFixed(0)} ms  conv-only RTFx ~${rtfx.toFixed(1)}x`);
  return gfs;
}

console.log(`Kokoro cost model — dominant conv ${Cin}×${L}×${Cout} K${K} (${gflop1.toFixed(2)} GFLOP); full-model conv ≈ ${CONV_GFLOP_TOTAL} GFLOP for ${AUDIO_S}s audio (decoder = ~90% of compute).\n`);
...
console.log(`Verdict: naive conv loses; im2col+GEMM is competitive. A register-blocked/fp16 GEMM (naive tiled GEMM is ~10% of fp32 peak) is the lever to win decisively.`);
process.exit(0);
