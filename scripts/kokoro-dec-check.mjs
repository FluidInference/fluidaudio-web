...
const anchorFor = {
  encode: "dec__decoder_decoder_encode_Div_3_output_0.bin",
  "decode.0": "dec__decoder_decoder_decode_0_Div_3_output_0.bin",
  "decode.1": "dec__decoder_decoder_decode_1_Div_3_output_0.bin",
  "decode.2": "dec__decoder_decoder_decode_2_Div_3_output_0.bin",
  "decode.3": "dec__decoder_decoder_decode_3_Div_4_output_0.bin",
};
const onStage = async (name, t, extra) => {
  if (extra) {
    for (const [k, v] of Object.entries(extra)) console.log(`  ${k}: [${v.rows},${v.cols}]`);
  }
  const ref = rd(`${K0}/${anchorFor[name]}`);
  const o = await ctx.download(t);
  const m = Math.min(o.length, ref.length);
  let md = 0;
  for (let i = 0; i < m; i++) md = Math.max(md, Math.abs(o[i] - ref[i]));
