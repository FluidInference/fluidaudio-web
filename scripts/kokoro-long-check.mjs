...
const map = { "kokoro/weights.bin": `${K0}/kw/weights.bin`, "kokoro/manifest.json": `${K0}/kw/manifest.json`, "kokoro/roles.json": `${K0}/kw/roles.json`, "kokoro/be_w.bin": `${K0}/frontend/be_w.bin`, "kokoro/be_b.bin": `${K0}/frontend/be_b.bin`, "kokoro/ref.json": `${K0}/frontend/ref.json`, "voices/af_heart.bin": `${K0}/af_heart.bin` };
const hfUrl = (r, p) => p;
const fetchCached = async (p) => { if (map[p]) return Uint8Array.from(readFileSync(map[p])); if (p.startsWith("kokoro/albert/")) return Uint8Array.from(readFileSync(`${K0}/albert/${p.split("/").pop()}`)); throw new Error("no map " + p); };
const be = await loadKokoroBackend(fetchCached, hfUrl, vocab);
// long sentence via lexicon
const lex = await EnglishLexicon.load(async (url) => { throw new Error("no net: " + url); }).catch(() => null);
...
let nan = false, peak = 0, s = 0;
for (let i = 0; i < wav.length; i++) { if (Number.isNaN(wav[i])) nan = true; peak = Math.max(peak, Math.abs(wav[i])); s += wav[i] ** 2; }
...
const h = Buffer.alloc(44); h.write("RIFF", 0); h.writeUInt32LE(36 + i16.byteLength, 4); h.write("WAVE", 8); h.write("fmt ", 12); h.writeUInt32LE(16, 16); h.writeUInt16LE(1, 20); h.writeUInt16LE(1, 22); h.writeUInt32LE(24000, 24); h.writeUInt32LE(48000, 28); h.writeUInt16LE(2, 32); h.writeUInt16LE(16, 34); h.write("data", 36); h.writeUInt32LE(i16.byteLength, 40);
