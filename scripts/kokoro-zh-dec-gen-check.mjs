...
let dm = 0; for (let i = 0; i < Math.min(d3.length, d3ref.length); i++) dm = Math.max(dm, Math.abs(d3[i] - d3ref[i]));
...
let dot = 0, a = 0, b = 0, mx = 0;
for (let i = 0; i < m; i++) { dot += wav[i] * ref[i]; a += wav[i] ** 2; b += ref[i] ** 2; mx = Math.max(mx, Math.abs(wav[i] - ref[i])); }
