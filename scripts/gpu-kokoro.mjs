...
const A = "/tmp/kokoro/albert", F = "/tmp/kokoro/frontend";
...
const albertW = {
  ...
  attn_ln_w: up1("attn_ln_w"), attn_ln_b: up1("attn_ln_b"), full_ln_w: up1("full_ln_w"), full_ln_b: up1("full_ln_b"),
  word_emb: cpu.word_emb, pos_emb: cpu.pos_emb, tok_emb: cpu.tok_emb, emb_ln_w: cpu.emb_ln_w, emb_ln_b: cpu.emb_ln_b,
};
...
let mx = 0, se = 0, sr = 0;
for (let i = 0; i < den.length; i++) { const d = Math.abs(den[i] - refDen[i]); mx = Math.max(mx, d); se += d * d; sr += refDen[i] * refDen[i]; }
