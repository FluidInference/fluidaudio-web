...
const beW = ctx.upload(rd(`${Z}/kw/be_w.bin`), ref.be_in, ref.be_out),
  beB = ctx.upload(rd(`${Z}/kw/be_b.bin`), 1, ref.be_out);
const idb = Uint8Array.from(readFileSync(`${Z}/zh_ids.bin`));
...
const refd = rd(`${Z}/zh_den.bin`);
let m = 0,
  se = 0,
  sr = 0;
for (let i = 0; i < den.length; i++) {
  m = Math.max(m, Math.abs(den[i] - refd[i]));
  se += (den[i] - refd[i]) ** 2;
  sr += refd[i] ** 2;
}
