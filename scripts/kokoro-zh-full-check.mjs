...
const beW = ctx.upload(rd(`${Z}/kw/be_w.bin`), fref.be_in, fref.be_out), beB = ctx.upload(rd(`${Z}/kw/be_b.bin`), 1, fref.be_out);

const _ib = readFileSync(`${Z}/zh_ids.bin`); const ids = new Int32Array(_ib.buffer, _ib.byteOffset, _ib.byteLength / 4);
...
// spec magnitude gate (my source vs ORT source; phase random by design)
{
  ...
  const ref = rd(`${Z}/anc_spec.bin`); const Tr = ref.length / 22, Tt = spec.cols;
  let dot = 0, a = 0, b = 0;
  const Tm = Math.min(Tt, Tr);
  for (let k = 0; k < 11; k++) for (let t = 0; t < Tm; t++) { const x = spec.data[k * Tt + t], y = ref[k * Tr + t]; dot += x * y; a += x * x; b += y * y; }
...
const rms = (x) => { let s = 0; for (let i = 0; i < x.length; i++) s += x[i] ** 2; return Math.sqrt(s / x.length); };
...
h.writeUInt32LE(36 + r16.byteLength, 4); h.writeUInt32LE(r16.byteLength, 40);
