...
function readWav(p) {
  ...
  if (bits !== 16) throw new Error("bits=" + bits);
  const n = dL / 2,
...
function wer(ref, hyp) {
  const r = norm(ref),
    ...
    n = r.length,
    m = h.length;
  let d = Array.from({ length: m + 1 }, (_, i) => i);
  for (let i = 1; i <= n; i++) {
    let prev = d[0];
    d[0] = i;
    for (let j = 1; j <= m; j++) {
      const cur = d[j];
      d[j] = Math.min(d[j] + 1, d[j - 1] + 1, prev + (r[i - 1] !== h[j - 1] ? 1 : 0));
      prev = cur;
    }
  }
  return { err: d[m], n };
...
for (const line of readFileSync(`${FLEURS}/en_us.trans.txt`, "utf8").split(/\r?\n/)) {
  ...
  refs.set(line.slice(0, sp), line.slice(sp + 1));
}
...
for (const id of ids) {
  ...
  const { err, n } = wer(refs.get(id), r.text);
  totErr += err;
  totN += n;
  totDur += samples.length / sampleRate;
  totMs += r.metrics.totalMs;
}
