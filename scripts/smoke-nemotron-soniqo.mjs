...
const pp = new JsPreprocessor({ nMels: 128 });
const { features } = pp.process(audio);
const T = features.length / 128;
const F = 32,
  M = 128;
const lm = new Float32Array(128);
lm[0] = 1;
let pre = new Float32Array(M * 9),
  clc = new Float32Array(24 * 56 * 1024),
  clt = new Float32Array(24 * 1024 * 8),
  clen = 0;
// RNNT state
let token = BLANK,
  h = new Float32Array(2 * 640),
  c = new Float32Array(2 * 640);
const ids = [];
const nChunks = Math.ceil(T / F);
for (let ci = 0; ci < nChunks; ci++) {
  const chunk = new Float32Array(M * F);
  const valid = Math.min(F, T - ci * F);
  ...
  // carry caches
  pre = o["new_pre_cache"].data.slice();
  const nc = o["new_cache_last_channel"];
  clc = nc.data.slice();
  clt = o["new_cache_last_time"].data.slice();
  clen = Number(o["new_cache_last_channel_len"].data[0]);
  ...
  const eo = o["encoded_output"].data;
  const nf = o["encoded_output"].dims[1];
  for (let f = 0; f < Math.min(nf, nlen || nf); f++) {
    const encF = new Float32Array(1024);
    for (let d = 0; d < 1024; d++) encF[d] = eo[f * 1024 + d];
    let emitted = 0;
    while (emitted < 10) {
