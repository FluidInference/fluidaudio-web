...
let fails = 0;
const report = (name, err, tol) => {
  const ok = err <= tol;
  if (!ok) fails++;
  console.log(`${ok ? "✓" : "✗"} ${name.padEnd(28)} max|wasm-cpu| = ${err.toExponential(2)}  (tol ${tol.toExponential(0)})`);
};
const dl = (t) => t.data;
    ...
    for (let lo = 0; lo < Lout; lo++) {
      ...
      for (let ci = 0; ci < cinG; ci++) {
        ...
        for (let k = 0; k < K; k++) {
          ...
          if (li >= 0 && li < L) acc += X[rc * L + li] * W[(co * cinG + ci) * K + k];
        }
      }
      Y[co * Lout + lo] = acc;
    }
...
for (const cfg of [
  ...
  report(
    ...
    maxErr(
      dl(
        ctx.conv1d(ctx.upload(X, Cin, L), ctx.upload(W, 1, W.length), {
          cout: Cout,
          k: K,
...
// layernorm
{
  const R = 64,
    ...
    b = rand(C),
    eps = 1e-5,
    ref = new Float32Array(R * C);
  for (let r = 0; r < R; r++) {
    let m = 0;
    for (let j = 0; j < C; j++) m += X[r * C + j];
    m /= C;
    let v = 0;
    for (let j = 0; j < C; j++) v += (X[r * C + j] - m) ** 2;
    v /= C;
    const inv = 1 / Math.sqrt(v + eps);
    for (let j = 0; j < C; j++) ref[r * C + j] = (X[r * C + j] - m) * inv * g[j] + b[j];
  }
...
// add / mul (broadcast)
{
  const R = 40,
    ...
    A = rand(R * C),
    bc = rand(C),
    refA = new Float32Array(R * C),
    refM = new Float32Array(R * C);
  for (let i = 0; i < R * C; i++) {
    refA[i] = A[i] + bc[i % C];
    refM[i] = A[i] * bc[i % C];
  }
...
// matmulNBits (int4, K%32≠0)
{
  ...
  report(
    "matmulNBits (int4)",
    maxErr(dl(ctx.matmulNBits(ctx.upload(A, M, K), ctx.uploadBytes(Bq), ctx.upload(scales, 1, scales.length), ctx.uploadBytes(zpU8), N)), ref),
    1e-3,
  );
}
// conv1dGemm + conv1dFast match direct conv
{
  ...
  report(
    "conv1dFast (fused)",
    maxErr(dl(ctx.conv1dFast(ctx.upload(X, Cin, L), ctx.upload(W, Cout, Cin * K), Cout, K, { pad, bias: ctx.upload(bias, 1, Cout) })), ref),
    1e-3,
  );
}
// gatherCols
{
  const C = 16,
    T = 8,
    X = rand(C * T),
    idx = Uint32Array.from([0, 0, 0, 1, 2, 2, 3, 4, 4, 4, 4, 7]);
      ...
      for (let ci = 0; ci < cinG; ci++) {
        ...
        for (let k = 0; k < K; k++) {
          ...
          if (num >= 0 && num % stride === 0) {
            const li = num / stride;
            if (li >= 0 && li < L) acc += X[rc * L + li] * W[rc * (coutG * K) + coInG * K + k];
          }
        }
      }
      Y[co * Lout + lo] = acc;
...
// bidirectional LSTM
{
  ...
  const X = rand(seq * inp),
    ...
    R = rand(2 * 4 * hid * hid),
    B = rand(2 * 8 * hid),
    ref = new Float32Array(seq * 2 * hid);
  for (let dir = 0; dir < 2; dir++) {
    const wB = dir * 4 * hid * inp,
      rB = dir * 4 * hid * hid,
      bB = dir * 8 * hid,
      h = new Float32Array(hid),
  ...
  report(
    "lstm (bidir, iofc)",
    maxErr(dl(ctx.lstm(ctx.upload(X, seq, inp), ctx.upload(W, 1, W.length), ctx.upload(R, 1, R.length), ctx.upload(B, 1, B.length), hid)), ref),
    1e-4,
  );
}
// adain
{
  const C = 40,
    ...
    sc = rand(C),
    sh = rand(C),
    ref = new Float32Array(C * L);
  for (let ch = 0; ch < C; ch++) {
    let m = 0;
    for (let j = 0; j < L; j++) m += X[ch * L + j];
    m /= L;
    let v = 0;
    for (let j = 0; j < L; j++) v += (X[ch * L + j] - m) ** 2;
    v /= L;
    const inv = 1 / Math.sqrt(v + eps);
    for (let j = 0; j < L; j++) ref[ch * L + j] = (X[ch * L + j] - m) * inv * sc[ch] + sh[ch];
  }
...
// transpose / sliceCols / setCols
{
  ...
  const dst = ctx.upload(new Float32Array(R * C), R, C),
    src = ctx.upload(rand(R * W), R, W);
  ctx.setCols(dst, src, off);
  let dErr = 0;
  for (let r = 0; r < R; r++) for (let j = 0; j < W; j++) dErr = Math.max(dErr, Math.abs(dst.data[r * C + off + j] - src.data[r * W + j]));
...
// subReshape / jbatch / argmaxRows / jointArgmax
{
  const C = 8,
    ...
    F = 4,
    X = rand(C * Tsub * F),
    ref = new Float32Array(Tsub * C * F);
  for (let idx = 0; idx < Tsub * C * F; idx++) {
    const CF = C * F,
      ho = Math.floor(idx / CF),
      rem = idx % CF,
      c = Math.floor(rem / F),
      wo = rem % F;
    ref[idx] = X[c * (Tsub * F) + ho * F + wo];
  }
...
}
{
  const Tenc = 10,
    ...
    B = 4,
    base = 2,
    encProj = rand(Tenc * hid),
    predProj = rand(hid),
    ref = new Float32Array(B * hid);
...
}
{
  const Tenc = 5,
    ...
    count = 3,
    frame = 1;
  const encProj = rand(Tenc * hidden),
    predProj = rand(hidden),
    outW = rand(hidden * logits),
    outB = rand(logits),
    ref = new Float32Array(count * 4);
  for (let w = 0; w < count; w++) {
    const base = (frame + w) * hidden,
      j = new Float32Array(hidden);
    for (let k = 0; k < hidden; k++) j[k] = Math.max(0, encProj[base + k] + predProj[k]);
    let bt = 0,
      bv = -1e30,
      bd = 0,
      bdv = -1e30;
    for (let n = 0; n < logits; n++) {
      let s = outB[n];
      for (let k = 0; k < hidden; k++) s += j[k] * outW[k * logits + n];
      if (n < vocab) {
        if (s > bv) {
          bv = s;
          bt = n;
  ...
  report(
    ...
    maxErr(
      dl(
        ctx.jointArgmax(
          ctx.upload(encProj, Tenc, hidden),
          frame,
          count,
          ctx.upload(predProj, 1, hidden),
          ctx.upload(outW, hidden, logits),
          ctx.upload(outB, 1, logits),
          hidden,
          vocab,
          logits,
        ),
