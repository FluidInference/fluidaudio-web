...
import { predictor, sineGen, sourceSpec, decoder } from "../src/gpu/kokoro-synth.js";
{
  ...
  const rdc = rd(`${K0}/real_decode3.bin`);
  let dm = 0;
  for (let i = 0; i < Math.min(d3.length, rdc.length); i++) dm = Math.max(dm, Math.abs(d3[i] - rdc[i]));
  ...
  const corrOf = (lo, hi) => {
    ...
    for (let ch = lo; ch < hi; ch++)
      for (let t = 0; t < Tt; t++) {
        const a = spec.data[ch * Tt + t],
          b = c3[ch * 10681 + t];
        dot += a * b;
        na += a * a;
        nb += b * b;
      }
    return (dot / Math.sqrt(na * nb)).toFixed(4);
  };
  ...
  for (let k = 0; k < 11; k++)
    for (let t = 0; t < Tt; t++) {
      const ma = spec.data[k * Tt + t],
        pa = spec.data[(k + 11) * Tt + t],
        mb = c3[k * 10681 + t],
        pb = c3[(k + 11) * 10681 + t];
      const ar = ma * Math.cos(pa),
        ai = ma * Math.sin(pa),
        br = mb * Math.cos(pb),
        bi = mb * Math.sin(pb);
      dot += ar * br + ai * bi;
  ...
  for (let n = 0; n < NF; n++) win[n] = 0.5 - 0.5 * Math.cos((2 * Math.PI * n) / NF);
  const oo = new Float32Array(Lo),
    ws = new Float32Array(Lo);
  for (let t = 0; t < Tc3; t++) {
    for (let n = 0; n < NF; n++) {
      let v = 0;
      for (let k = 0; k < NF; k++) {
        const kk = k <= 10 ? k : NF - k;
        const sgn = k <= 10 ? 1 : -1;
        const mag = c3[kk * Tc3 + t],
          ph = sgn * c3[(kk + 11) * Tc3 + t];
        v += mag * Math.cos((2 * Math.PI * k * n) / NF + ph);
      }
      v /= NF;
      oo[t * hop + n] += v * win[n];
      ws[t * hop + n] += win[n] * win[n];
    }
  }
  ...
  let sc = 0,
    mm = 0,
    oo2 = 0;
  const mn = Math.min(ortSrc.length, mySrc.length);
  for (let i = 0; i < mn; i++) {
    sc += mySrc[i] * ortSrc[i];
    mm += mySrc[i] ** 2;
    oo2 += ortSrc[i] ** 2;
  ...
  console.log(
    `SOURCE time-domain: my-vs-ORT(iSTFT c3) corr ${(sc / Math.sqrt(mm * oo2)).toFixed(4)}  rms my ${Math.sqrt(mm / mn).toFixed(4)} ort ${Math.sqrt(oo2 / mn).toFixed(4)}`,
  );
}
const t0 = Date.now();
...
hdr.write("RIFF", 0);
hdr.writeUInt32LE(36 + i16.byteLength, 4);
hdr.write("WAVE", 8);
hdr.write("fmt ", 12);
...
hdr.write("data", 36);
hdr.writeUInt32LE(i16.byteLength, 40);
