...
async function t(fn,M,K,N,iters=100){
  ...
  for(let w=0;w<8;w++)fn(a,b); await done();
  const t0=performance.now(); for(let i=0;i<iters;i++)fn(a,b); await done();
  const ms=(performance.now()-t0)/iters; return 2*M*K*N/(ms/1e3)/1e12;
}
// real speech-model shapes (M×K×N)
const shapes=[[200,512,512],[200,512,2048],[1500,512,512],[1500,512,2048],[512,1024,1024],[512,1024,4096]];
