...
console.log("T =", out.T, "predDur", Array.from(out.predDur), "\nORT dur:", Array.from(refDur).map(Math.round));
const me = async (t, refPath, name) => { const o = t.data ? t.data : await ctx.download(t); const ref = rd(refPath); const m = Math.min(o.length, ref.length); let d = 0; for (let i = 0; i < m; i++) d = Math.max(d, Math.abs(o[i] - ref[i])); console.log(`  ${name}: [${t.rows},${t.cols}] (${o.length}) vs ref ${ref.length}  maxΔ ${d.toExponential(2)}`); return d; };
let bad = 0;
