...
const SPK=4;
async function runWindow(samples){const {features,length}=mel.process(samples);const r=await parakeetEncode(ctx,enc,features,length);const preds=await sortformerHead(ctx,head,r.framesGpu,r.Tsub);console.log(`   win ${(samples.length/16000).toFixed(0)}s → T=${r.Tsub} predsMax=${Math.max(...preds).toFixed(3)}`);return {preds,frames:preds.length/SPK};}
async function diarize(samples,sr=16000){
  const WIN=90*sr, OVL=15*sr, hop=WIN-OVL;
  if(samples.length<=WIN){const w=await runWindow(samples);return predsToSegments(w.preds,w.frames,samples.length/sr/w.frames);}
  const windows=[],ovlF=[];
  for(let s=0;s<samples.length;s+=hop){const end=Math.min(s+WIN,samples.length);const w=await runWindow(samples.subarray(s,end));const fps=w.frames/((end-s)/sr);windows.push(w);ovlF.push(Math.round((OVL/sr)*fps));if(end>=samples.length)break;}
...
console.log("40s single-window:"); console.log(" ", fmt(await diarize(one)));
