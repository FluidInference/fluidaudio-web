...
const T = 188, D = 1024, DFF = 4096, H = 8, HD = 64, P = 2 * T - 1;
async function t(name, fn, reps) {
  for (let i = 0; i < 3; i++) fn(); await done();
  const t0 = performance.now();
  ctx.beginBatch(); for (let i = 0; i < reps; i++) fn(); ctx.endBatch();
  await done();
...
const q8 = new Int8Array(D * DFF); for (let i = 0; i < q8.length; i++) q8[i] = (Math.random() * 200 - 100) | 0;
const wq = ctx.uploadBytes(new Uint8Array(q8.buffer)); const sc = ctx.upload(rand(DFF), 1, DFF);
...
const q82 = new Int8Array(DFF * D); for (let i = 0; i < q82.length; i++) q82[i] = (Math.random() * 200 - 100) | 0;
const wq2 = ctx.uploadBytes(new Uint8Array(q82.buffer)); const sc2 = ctx.upload(rand(D), 1, D);
const mFF2 = await t("int8 GEMM 188x4096x1024 (v2)", () => ctx.matmulInt8(a2, wq2, sc2, D, DFF), 50);
// attention batched
const qq = ctx.upload(rand(T * D), T, D), kk = ctx.upload(rand(T * D), T, D), vv = ctx.upload(rand(T * D), T, D);
const pp = ctx.upload(rand(P * D), P, D), qb = ctx.upload(rand(D), 1, D);
