...
{
  const dec3 = rd(`${K0}/real_decode3.bin`),
    c3 = rd(`${K0}/c3.bin`),
    style = rd(`${K0}/hello_style.bin`).slice(0, 128);
...
// predictor gate (ref anchors)
{
  ...
  const f0 = await ctx.download(out.F0),
    asr = await ctx.download(out.asr);
  console.log(
    `GPU predictor: F0 maxΔ ${me(f0, rd(`${K0}/anc__encoder_F0_proj_Conv_output_0.bin`)).toExponential(2)}  asr maxΔ ${me(asr, rd(`${K0}/anc__encoder_MatMul_1_output_0.bin`)).toExponential(2)}  xC maxΔ ${me(out.xConcat.data, rd(`${K0}/anc__decoder_decoder_Concat_output_0.bin`)).toExponential(2)}`,
  );
}
process.exit(0);
