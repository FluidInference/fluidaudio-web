...
const map = {
  ...
  "kokoro/albert/manifest.json": `${K0}/albert/manifest.json`,
...
const fetchCached = async (path) => {
  ...
  if (path.startsWith("kokoro/albert/")) return Uint8Array.from(readFileSync(`${K0}/albert/${path.split("/").pop()}`));
...
const pack = rd(`${K0}/af_heart.bin`);
const si = 256 * Math.min(Math.max(ids.length - 2, 0), 509);
const hs = rd(`${K0}/hello_style.bin`);
let sd = 0;
for (let i = 0; i < 256; i++) sd = Math.max(sd, Math.abs(pack[si + i] - hs[i]));
